/**
 * v1 feature handler — retrospect action（查 child slice 状态 + 跑 7 个 gate + 写 retrospectData）。
 *
 * 设计来源：feature-internal.runFeatureRetrospectGates（7 个 gate 聚合，复用 slice 的 7 个子 gate）、
 ...
 * 3. 跑 runFeatureRetrospectGates(unit, childStatuses)（7 个 gate：allWavesClosed + lessons + cover + splitFulfillment + childUnitEvidence + deliveryVerdict + childDeliveryConsistency）
...
export function handleRetrospectFeature(
  ...
  validateInput("retrospect", "feature", input);
  ...
  const gateResults = runFeatureRetrospectGates(unit, childStatuses, evidenceChildDelivery);
