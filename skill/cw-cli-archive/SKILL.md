...
name: cw-cli-archive
description: >-
  [ARCHIVED] cw 0.x CLI skill — 旧版单层 topic 编码流程（taskShape/19 action/熔断等）。
  已被 cw-cli（v1 版，cw v1 create <layer> + 渐进式 guidance）取代。
  保留此 skill 供 0.x 命令向后兼容参考。新任务请用 cw-cli。
---

> **归档说明**：本 skill 是 cw 0.x 的编码流程指南。v1 已用 4 层 WorkUnit 模型
> （epic/feature/slice/wave）+ 渐进式 guidance 取代了 0.x 的单层 topic 模型。
> 0.x 代码已迁移到 `src/legacy/`，命令仍可用（`cw create` 不带 v1 前缀）。
> **新任务请用 `cw v1 create`（见 cw-cli skill）**。
