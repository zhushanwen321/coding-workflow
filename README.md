...
Agent-agnostic CLI for the Coding Workflow (CW) engine — a state machine that keeps AI coding agents on track through a `create → plan → dev → test → retrospect → closeout` pipeline.
...
# → 全部 Wave committed 后 nextAction.action=test
...
# 5. 写复盘报告，提交路径
cw retrospect --topicId <topicId> --retrospect-path .xyz-harness/my-feature/retrospect.md
# → nextAction.action=closeout

# 6. 归档
cw closeout --topicId <topicId> --evidence "69 tests passed"
...
| `retrospect` | `--topicId`, `--retrospect-path` | — | 提交复盘报告路径 |
| `closeout` | `--topicId` | `--evidence` | 归档 topic，流程终态 |
| `replan` | `--topicId` | stdin 或 `--replanJsonFile` | dev 阶段追加 wave（append-only） |
...
created → planned → developed → tested → retrospected → closed
              ↑         |                    
              └─ replan ┘（dev 阶段追加 wave，回退到 planned）
```
...
npm test         # vitest run（69 个测试：单测 + 真实子进程 e2e）
...
├── actions.ts          # 7 个 action handler + CwParams 联合类型
