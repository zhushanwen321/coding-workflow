...
| **Action** | CW 接受的 15 种操作之一（见下表） | `src/cli.ts:151-167` |
...
> ADVANCE_ACTIONS = create + 9 推进（`src/cli.ts:151`）；READONLY_QUERIES = 5 只读（`src/cli.ts:167`）。
> test / exec-review 是 wave 专属——PlanningUnit 收到会抛 illegal_transition。

## 两层 Status 枚举

### PlanningStatus（epic/feature/slice，7 态）

```
created → designing → design-reviewed → executing → retrospected → closed
                                                ↘ aborted（终态）
```

出处：`src/core/status.ts:17`。

### ExecutionStatus（wave，9 态）
...
出处：`src/core/status.ts:27`。wave 比 PlanningUnit 多 `tested` 和 `exec-reviewed`（在 executing 与 retrospected 之间）——因为只有 wave 产代码需要 test/exec-review。

终态：`closed` / `aborted`（不可逆）。出处：`src/core/status.ts:120`（TERMINAL_STATUSES）。
...
关键转换（完整表见 `src/rules/state-machine.ts:63` WAVE_TRANSITIONS / `:284` PLANNING_TRANSITIONS）：

| action | wave from → to | planning from → to |
|--------|---------------|-------------------|
| design | created,designing,design-reviewed → designing | 同 |
| design-review | designing,design-reviewed → design-reviewed | 同 |
...
| **ActionResult** | engine 统一返回：unitId/status/ok/gateResults?/nextAction?/failureCount?/children? | `src/handlers/types.ts:106` |
| **nextAction** | engine 返回的导航信息：action? + guidance + unitPath + crossLayer? | `src/handlers/types.ts:158` CwNextAction |
...
| **CwDeps** | engine 依赖注入接口：store/gitValidator/testRunner?/fileExists/workspacePath/clock | `src/handlers/types.ts:78` |
| **CwStore** | store.json 持久化层：POSIX 原子写 + 跨进程文件锁 + 内存事务（深拷贝+ROLLBACK） | `src/store/cw-store.ts:72` |
...
各层差异：epic/feature/slice 是 PlanningUnit（含 designReviewJudgment/PlanningEvidence/PlanningRetrospectData）；wave 是 ExecutionUnit（额外含 testJudgment/execReviewJudgment/WaveEvidence/RetrospectData）。feature 的 design 产物是容器对象（含 spec），其余三层是裸数组。
...
出处：`src/core/plan.ts:36`（Plan 基类）、`:66`（WavePlan）、`:132`（SlicePlan）。
...
| 中间产物 | `<workspacePath>/.cw/<slug>/<action>.json` | design/design-review 等阶段 input JSON（已 gitignore） |
...
**做什么**：用状态机 + 机器检查 gate 强制 AI 编码任务走结构化流程（design → design-review → execute → test → exec-review → retrospect → closeout）。
