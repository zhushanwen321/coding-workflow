...
| `execute` | 线性 | wave: 记录 commitHash；PlanningUnit: 按 split 下沉创建子 unit（nextAction 填 parallelTargets 就绪批次 + crossLayer.descend 绑定首个就绪 child） |
...
| **ActionResult** | engine 统一返回：unitId/status/ok/gateResults?/nextAction?/failureCount?/children?/parallelTargets? | `src/handlers/types.ts:87` |
| **nextAction** | engine 返回的导航信息：action? + guidance + unitPath + crossLayer? + parallelTargets? | `src/handlers/types.ts:126` CwNextAction |
| **guidance** | 拼入 nextAction 的纯文本提示词，agent 的唯一导航来源；正常三段式/异常四段式（并行场景额外渲染「## 并行调度」段） | `src/guidance/build-guidance.ts` |
| **crossLayer** | closeout 后跨层导航：descend（下沉首个就绪子）/ sibling（横向首个就绪兄弟）/ ascend（回父 retrospect）；单值，向后兼容串行导航 | `src/guidance/cross-layer.ts:60` |
| **parallelTargets** | CwNextAction 新增可选字段：execute 下沉 / closeout 回溯时填的「就绪子 unit 并行批次」（依赖已满足的非终态 child 列表）。与 crossLayer 一致性规则：非空时 crossLayer 锚定 parallelTargets[0]；undefined 时退化为现有串行行为。详见 [ADR-0011](./docs/adr/0011-recursive-parallel-scheduling.md) | `src/handlers/types.ts:173` |
