...
> 本文档只讲 feature 一层。流程结构与 epic 层**完全相同**（同样的 8 步、同样的状态机、同样的命令约定），差异只在每步的具体内容——feature 的 **clarify 写需求 spec**（FR/AC/UC，存 `clarification.spec`），**plan 拆 slice**（`plan.split`），verify 做结构化业务判断，execute **启动 slice 层**。
>
> **顶层概念体系**（和 epic 一致）：使用者的认知模型只有两个维度——**4 个层**（epic/feature/slice/wave）× **8 个步骤**（create/clarify/plan/verify/execute/test/retrospect/closeout）。每一步在每一层都有一个**同名产物**，需要区分时加层前缀。`Clarification` / `SpecSection` / `FR-AC` 这些专有名词降级为各层 clarify/plan 的**内部字段**，不作为顶层概念暴露。
>
...
>
> **和 epic 严格同构**：机制层面（replan、abandonedRefs、reviewedItems、basedOnParent、verifyJudgment/testJudgment/retrospectData）完全一致。差异在产物边界：feature 的 **spec 在 clarify 阶段产出**（`clarification.spec`，FR/AC/UC），**plan 只产出 split**（拆 slice，直接用 `Plan` 基类，不单独定义 `FeaturePlan`）——这是 v4 三期相对二期的关键调整，spec 不再是 plan 的子字段。学一层，会四层。
...
epic 「重构认证系统」
  └─ feature 「OAuth 登录」       ← 本文讲这一层
       ├─ clarify: 澄清需求细节 + 写 spec（FR/AC/UC，存 clarification.spec）
       ├─ plan:    拆 slice（plan.split）
...
| | epic | feature |
|---|---|---|
| **clarify 产物** | `Clarification`（架构方向：用 OAuth 还是 session？）| `Clarification`（需求细节：支持哪些登录方式？错误态怎么展示？）+ **`clarification.spec`**（FR/AC/UC 规格化条目）|
| **plan 产物** | `Plan.split`（拆 feature 清单 + 依赖 DAG，直接用基类）| **`Plan.split`**（拆 slice 清单 + 依赖 DAG，直接用基类。spec 不在 plan 里，在 clarification.spec）|
...
### 4.2 第 2 步：clarify（澄清需求细节 + 写 spec）

**做什么**：识别这个 feature 的需求要定清楚，必须先回答的问题（**Clarification**——clarify 阶段的产物），逐个给答案；并把答案**规格化成 spec**（SpecSection 集合）。feature 的 clarify 阶段产出两样东西，统一存在 `feature.clarification` 容器里：

- `clarification.clarifications`：问题 + 答案（机制和字段同 epic §5.2 / §7.2，不再重复）
- `clarification.spec`：基于答案规格化出来的条目（FR/AC/UC 等，详见 §5）

> **v4 三期边界划分**：feature 的 **spec 在 clarify 阶段产出**（`clarification.spec`），**不在 plan 里**。这是 v4 三期相对二期的关键调整——原 `featurePlan.spec` 迁到 `clarification.spec`，让 plan 只剩 split（拆 slice），feature 直接用 `Plan` 基类，不需要单独的 `FeaturePlan` 子类型。理由：spec 是「规格化需求」的产物（和 questions/answers 同源，是 clarify 的自然延展），plan 是「拆下一层」的产物（split），两者职责清楚分开。
>
> **feature 的 clarification 是容器对象（其他层是数组）**：feature 有 `clarification.spec`（FR/AC/UC 等），所以 feature 的 `clarification` 字段是 `{ clarifications, spec }` 容器对象；epic/slice/wave 没有 spec 子结构，直接用 `clarifications: Clarification[]` 数组。读取时注意：feature 走 `workUnit.clarification.clarifications`，其他层走 `workUnit.clarifications`。
...
**和 epic clarify 的区别**：epic 澄清的是**架构方向**（用 OAuth 还是 session），feature 澄清的是**需求细节**（支持哪些提供商、错误态怎么分），并把答案规格化成 spec 的 SpecSection（spec 的 `decisions` 章节直接引用 Clarification，避免双份记录）。epic 不写 spec（epic 没有 FR/AC/UC），feature 写 spec。

#### clarify 阶段的 spec 产物（`clarification.spec`：SpecSection 集合）
...
# 输入数据从 stdin 读：
# {
#   clarifications: [...],          # 问题+答案（存入 clarification.clarifications）
#   spec?: SpecSection[]            # 规格化条目（存入 clarification.spec，FR/AC/UC 等）
# }
# 注：spec 随 clarify 阶段 progressive 累积，agent 可先填 questions 再补 spec，或同步填
```

### 4.3 第 3 步：plan（拆 slice）

**做什么**：基于 clarify 阶段定下来的 spec，**拆 slice**。feature 的 plan 只产出一样东西：`plan.split`（拆下一层清单）。feature 直接用 `Plan` 基类（只含 split），不单独定义 `FeaturePlan`——这是 v4 三期的边界划分（spec 在 clarify，plan 只做 split）。

#### plan 的唯一产物：slice 拆分清单（`plan.split`）

基于 `clarification.spec`，把这个 feature 拆成若干 slice（split 在 plan 里做，作为 `plan.split` 内部字段，所有层 plan 共享 `Plan` 基类的 `split` 字段）：
...
> slice 负责哪些 FR/AC/UC 的引用关系不在 split 里存——slice create 时 cw 把 feature 当前的 FR/AC/UC id 全量拷贝到 slice.`basedOnParent`（slice plan 时可减少到只留自己真正负责的），机制见 §4.5。
>
...
**产出**：`plan`（`Plan` 基类，含 `split`——拆 slice 清单）
...
# 输入数据从 stdin 读：
# {
#   split: Split[]                  # slice 拆分清单（存入 plan.split）
# }
# 注意：spec 不在 plan 输入里——spec 是 clarify 阶段产出的（走 cw feature clarify 输入）
```

### 4.4 第 4 步：verify（spec 审查）

**做什么**：在启动 slice 层**之前**，对 clarify 阶段的 spec 和 plan 阶段的 slice 拆分做**结构化的业务判断**。机制同 epic verify（见 [epic 文档 §5.4](./design-v4-epic.md#54-第-4-步verify审查规划)）——cw 定义存储结构 + guidance 提示 + 结构校验，**业务判断内容由 agent 产出**。
...
1. 根据 plan 阶段的 `split` 拆分清单，为每个 slice 创建实例
   - 每个 slice 的 `parentUnitId` 指向这个 feature
   - 每个 slice 的 `basedOnParent` 从 feature 继承：cw 把 feature 当前的 Clarification id（`clarification.clarifications` 里的，feature 自己 clarify 产生的）+ feature spec 里的 FR/AC/UC id（`clarification.spec` 里的，**不含 Decision id**——Decision 是 Clarification 的 spec 层投影，跟随 Clarification replan，不独立持有 status/replacedBy，继承会制造无法作废的引用；对齐 slice 不让 wave 继承 Decision 的做法，详见 §5.4）**全量拷贝**到 slice.`basedOnParent`（slice plan 时可以减少到只留自己真正负责的）。**统一存在 `slice.basedOnParent`**，不区分 Clarification 引用和 spec 项引用两套字段（这是 v4 的统一语义，见 epic §7.3）
...
slice 不快照整个 spec，只记 `basedOnParent`（引用了 feature 的 clarify + plan 的哪些项——Clarification id + FR/AC/UC id 统一存在这一个字段里）。如果 feature 改了 spec（变更/撤销某个 FR/AC），走 **统一 replan**（见 §6）——feature 作废旧 FR/AC（`status=abandoned` + `replacedBy`），cw 反查子树里 `basedOnParent` 含被废弃 FR/AC id 的 slice，在它们的 `abandonedRefs` 追加记录，强制处理（accept-replan 或 abort）。机制层面和 epic 改 Clarification 完全一致。
...
## 5. SpecSection（clarification.spec 的组成单元）

### 5.1 SpecSection 是什么

SpecSection 是 `clarification.spec` 的组成单元（不再是顶层概念）。cw 0.x 已经定义了完整的 SpecSection 体系（`src/types.ts`），feature 直接继承，不重新发明。
...
**`decisions` 章节项的类型是跨层共享的 `Decision`**：feature spec 的 `decisions` 章节（`{ type: "decisions", items: Decision[] }`）里每一项的类型是 `Decision`——和 slice 的 `slicePlan.decisions` 共用同一个 `Decision` 类型（跨层共享，定义见附录）。`Decision` 是 Clarification 的 spec 层投影（`sourceClarification` 字段记录它投影自哪个 Clarification，id 直接对齐 Clarification 的 id），不继承 `WorkUnitItem`、不独立持有 `status` / `replacedBy`，跟随 Clarification replan。其他 SpecSection 类型（FR/AC/UC）继承 `WorkUnitItem`（独立 replan），`Decision` 例外——详见 §5.4。
...
export interface FunctionalRequirement extends WorkUnitItem {
  // id / status / replacedBy 继承自 WorkUnitItem（支持 replan 追踪）
...
和 epic 的 Clarification 一样，SpecSection **只是 feature clarify 阶段产生的列表项**，挂在 feature 的 `clarification` 容器里：

```
feature
  ├─ objective: "..."
  ├─ status: planning
  ├─ clarification: {               // clarify 阶段填（容器，含 questions + spec）
  │    clarifications: [...]        // feature 自己的 Clarification
  │    spec: [                      // clarification.spec（v4 三期从 plan 迁过来）
  ...
  │    ],
  │  }
  ├─ plan: {                        // plan 阶段填（Plan 基类，只含 split）
  │    split: [...]                 // plan.split（拆 slice 清单）
  │  }
...
- `abandonItems` 里填要作废的 FR/AC/UC id（epic 层填的是 Clarification id，feature 层填的是 FR/AC/UC id，**机制统一、只是 id 的语义不同**）。**不含 Decision id**——Decision 是 Clarification 的 spec 层投影，跟随 Clarification replan（Clarification 作废时 Decision 自动跟随），不独立持有 status/replacedBy。如果 slice 的 `basedOnParent` 继承了 Decision id，会制造「Decision 无法被 feature replan 作废但又被下游引用」的矛盾（对齐 slice 不让 wave 继承 Decision 的做法）。spec 变更走统一 replan 时，只作废 FR/AC/UC（以及 feature 自己的 Clarification）
...
| `SpecDecision`（cw 0.x decisions 章节的元素，`{id, decision, rationale}`）| **v4 三期统一命名为 `Decision`（跨层共享类型，feature 的 `clarification.spec` 和 slice 的 `slicePlan.decisions` 都用 `Decision`）**。cw 0.x 的 SpecDecision 定位为 Clarification 的 spec 层投影：decisions 章节不独立持有 status/replacedBy，它的内容引用 Clarification（通过 `sourceClarification?: string` 字段记录它投影自哪个 Clarification，id 直接对齐 Clarification 的 id）。Clarification 被 replan 时，decisions 章节自动跟随（不独立参与 replan）。这样避免「Clarification 能 replan 但 Decision 不能」的不一致。**关键**：Decision id 不被下游 slice 的 `basedOnParent` 继承（slice 只继承 Clarification + FR/AC/UC id）——因为 Decision 不独立持有 status/replacedBy，如果被下游引用会制造「无法被 feature replan 作废但又被 slice 引用」的矛盾。对齐 slice 不让 wave 继承 Decision 的做法。|

> **v4 三期命名统一**：原 feature 的 `SpecDecision` / slice 的 `TechDecision` / `SliceDecision` 三个名字统一为 `Decision`（跨层共享类型）。Decision 的 id 直接对齐 Clarification 的 id（如 `D3`），不再是 `TD1`/`TD2` 等独立编号。
>
> **本节标 TODO 的是实现侧迁移工作**，不影响 v4 设计文档的逻辑自洽。设计层面，v4 的 spec 变更机制是「逐项 replan + Decision 作为 Clarification 投影」，清晰且统一。

**Decision 的 `sourceClarification` 指向哪一层的 Clarification**：feature 的 Decision 投影自 **feature 自己 clarify 阶段产生的 Clarification**（存在 `clarification.clarifications` 里，不是从父 epic 继承的 Clarification）。理由：
- Decision 是 spec 阶段（feature 的 clarify 产物）的投影，只在产生它的那一层有效
- feature clarify 阶段回答的需求问题（如「token 存哪」），答案进 feature 自己的 `clarification.clarifications`，同时投影成 feature spec 的 `decisions` 项（`Decision.sourceClarification` 指向这条 Clarification）
- feature 从 epic 继承的 Clarification（存在 `basedOnParent`，是 epic 层的 Clarification id）是 feature 的上游约束，不是 feature 自己 clarify 的产物，feature 的 Decision 不投影它们——epic 的 Decision（如果有）投影自 epic 自己的 Clarification，是 epic 层的事，和 feature 的 Decision 各自独立
- 同一层的 Clarification id 和 Decision id 一致（如 feature 的 Clarification D3 对应 feature 的 Decision D3），不同层之间 id 独立（feature 的 D3 和 epic / slice 的 D3 是不同 WorkUnit 的不同条目，不冲突）
...
**已 closeout 的 slice**（边界场景）：feature replan 时如果某个 slice 已经 closed（真终态），cw 静默忽略（不追加 abandonedRefs、不记日志）——机制同 epic §8.2「已 closeout 的下游」段。这是 feature 层最常见的边界场景（一个 feature 拆了 N 个 slice，前 N-1 个 closeout 了，最后一个在跑，这时 feature 发现 spec 要改）。已交付的 slice 不动，真要跟进新决策由新 slice 重做。

**feature 的双向 replan 角色**（和 epic 不同，和 slice 对称）：feature 在 replan 机制中同时扮演两个角色——
- **作为发起者**：feature 改自己的 FR/AC/UC（上面描述的 4 步机制），标下游 slice
- **作为承受者**：被上游 epic replan 标记（epic 改 Clarification 触发下游 feature 的 abandonedRefs，refKind=`clarification`）。此时 feature 自己被阻塞，必须调 `cw feature accept-replan` 或 `cw feature abort` 解锁

这是 feature（和 slice）区别于 epic（只发起，不承受）的重要特性。epic 是顶层，basedOnParent 永远为空，永远是发起者。
...
#   输入数据从 stdin 读：{ clarifications: [...], spec?: [...] }
#   clarifications = 需求维度的澄清（支持哪些登录方式、错误态怎么分）
#   spec = 规格化条目（FR/AC/UC 等，存入 clarification.spec）

cw feature plan <id>                                 # progressive
#   输入数据从 stdin 读：{ split: [...] }
#   split = slice 拆分清单（存入 plan.split，Plan 基类字段）
#   注意：spec 不在 plan 输入里——spec 是 clarify 阶段产出的（走 cw feature clarify 输入）
...
#   根据 plan.split 创建 slice 实例，每个 slice 进入 created
#   每个 slice 的 basedOnParent 继承 feature 的 Clarification id + FR/AC/UC id
#   注意：不含 Decision id——Decision 是 Clarification 的 spec 层投影，跟随
#         Clarification replan，不独立持有 status/replacedBy，继承会制造无法作废的引用
#         （对齐 slice 不让 wave 继承 Decision 的做法，详见 §5.4）
...
cw feature show <id>                                 # 详情（含 clarification.spec + plan.split）
```
...
16. **spec 是 feature 的核心产物**：feature 的价值在于把 epic 的方向性决策翻译成 slice 能照着施工的需求规格。spec 写不好，下游 slice 全跟着跑偏。feature clarify 阶段在 spec 上花的时间是值得的。
...
interface Feature {
  ...
  // v4 三期边界划分：clarify 阶段产出 decisions + spec（规格化条目），plan 阶段只产出 split（拆下一层清单）
  // 所以 spec 在 clarification 容器里（feature.clarification.spec），不在 plan 里
  clarification: {                         // clarify 阶段填（容器，含 questions + spec）
    clarifications: Clarification[];       // feature 自己的 Clarification（原 decisionRecords）
    spec: SpecSection[];                   // 需求 spec（FR/AC/UC 等规格化条目，v4 三期从 plan 迁到 clarification）
  };
  plan?: Plan;                             // plan 阶段填（直接用 Plan 基类，只含 split，拆 slice）
...
// ── plan 产物（plan 阶段）──
// feature 直接用 Plan 基类（只含 split），不单独定义 FeaturePlan
// feature 的 spec 在 clarification.spec（不在 plan 里），这是 v4 三期的边界划分：
//   - clarification 阶段产出 decisions + spec（规格化条目）
//   - plan 阶段只产出 split（拆下一层清单）
// Plan / Split / WorkUnitItem 基类定义见 epic 文档附录（机制权威源）

// SpecSection 仍是类型（FR/AC/UC 等结构化章节），但不再是顶层概念，是 clarification.spec 的组成单元
type SpecSection = Feature['clarification']['spec'][number];
...
interface Clarification extends WorkUnitItem {
  ...
  // status: "active" | "abandoned"（v4 两值，继承 WorkUnitItem；原 cw 0.x 三值见 epic §5.2 修正说明）
}

// ── AbandonedRef（和 epic 同构，含 refKind 区分三种引用语义）──
interface AbandonedRef {
  refId: string;              // 被废弃的父层 plan 项 id（如 "D2" 或 "FR1"）
  refKind: "clarification" | "specItem" | "techItem";  // 引用类型：影响下游 accept 时的决策性质
...
// ── spec 内部结构（FR/AC/UC 等，在 clarification.spec，继承 WorkUnitItem 支持 replan）──

// FR（FunctionalRequirement）—— v4 新增 ac，继承 WorkUnitItem（拿 status/replacedBy）
export interface FunctionalRequirement extends WorkUnitItem {
  // id / status / replacedBy 继承自 WorkUnitItem（支持 replan）
  title: string;                         // "用户能用 GitHub 登录"
  detail: string;                        // 详细描述
  ac: string[];                          // 强引用 AC id（如 ["AC1.1", "AC1.2"]），让 fr-ac-coverage gate 机器可验
}

// AC（AcceptanceCriterion）—— 继承 WorkUnitItem（其他字段保持 cw 0.x 原样）
export interface AcceptanceCriterion extends WorkUnitItem {
  // id / status / replacedBy 继承自 WorkUnitItem
  condition: string;                     // 可判定的完成条件（cw 0.x 字段名，非 detail）
  verification?: "unit" | "manual" | "review";  // 如何验证（cw 0.x 可选字段）
}

// UC（BusinessCase）—— 继承 WorkUnitItem（其他字段保持 cw 0.x 原样）
...
// Decision（decisions 章节的项）——跨层共享类型，feature 的 clarification.spec 和 slice 的 slicePlan.decisions 都用
// 对应 cw 0.x 的 SpecDecision（v4 三期统一命名为 Decision）。是 Clarification 的 spec 层投影，跟随 Clarification replan
export interface Decision {
  id: string;                      // 直接用 Clarification 的 id，如 "D3"（id 和它投影的 Clarification 一致）
  decision: string;                // 决策结论
  ...
  // 引用 Clarification，跟随 Clarification replan（不独立参与 replan，Clarification 作废时 Decision 自动跟随）
  // 重要：Decision id 不被下游 slice 的 basedOnParent 继承（slice 只继承 Clarification + FR/AC/UC id），
  // 因为 Decision 不独立持有 status/replacedBy，被下游引用会制造「无法被 feature replan 作废但又被 slice 引用」的矛盾
  // （对齐 slice 不让 wave 继承 Decision 的做法，详见 §5.4）
}

// decisions 章节（SpecSection 的一种）：items 的类型是 Decision[]
//   { type: "decisions", items: Decision[] }
// 其他 SpecSection 类型（outOfScope / complexity / goals / background / constraints / section）
...
interface ClarifyInput {
  ...
  spec?: SpecSection[];          // 需求 spec（FR/AC/UC 等规格化条目，存入 clarification.spec）
                                  // v4 三期：spec 随 clarify 阶段产出（spec 是 clarify 的规格化条目，不是 plan 产物）
}

interface PlanInput {
  split: Split[];                // slice 拆分清单（存入 plan.split，Plan 基类字段）
  // 注意：spec 不在 PlanInput 里——spec 是 clarify 阶段产出的（存入 clarification.spec，走 ClarifyInput）
}

// replan 的输入数据（feature 层定义自己的判别联合）
// 同构的是 replan 的 4 步机制，不是 ReplanInput 类型集合。详见 epic 附录「每层定义自己的判别联合」说明
// feature 的 ReplacementContent = epic 的 Clarification kind + feature 扩展的 FR/AC/UC 三种 kind
interface FeatureReplanInput {
  abandonItems: Array<{
    id: string;
    refKind: "clarification" | "specItem" | "techItem";
    replacementContent?: FeatureReplacementContent;
  }>;
  newItems?: Array<FeatureNewItem>;
}

type FeatureReplacementContent =
  | { kind: "Clarification"; question: string; resolution?: string; type: "research" | "grilling" }  // 同 epic
  | { kind: "FR"; title: string; detail: string; ac: string[] }                                         // feature 扩展
  | { kind: "AC"; condition: string; verification?: "unit" | "manual" | "review" }                     // feature 扩展
  | { kind: "UC"; actor: string; scenario: string; expectedResult: string };                            // feature 扩展

type FeatureNewItem = FeatureReplacementContent;
// 注：slice 层的 SliceReplacementContent = FeatureReplacementContent 的所有分支 + slice 扩展的 TC/IF/DM/ERR 四种 kind（见 slice 文档附录）
...
const FEATURE_VERIFY_GATES = [
  ...
  "slice-split-non-empty",         // plan.split（拆 slice）至少 1 个
