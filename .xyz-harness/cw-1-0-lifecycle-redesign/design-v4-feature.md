...
1. 根据 plan 阶段的 `sliceSplit` 拆分清单，为每个 slice 创建实例
   ...
   - 每个 slice 的 `basedOnParent` 从 feature 继承：cw 把 feature 当前的 `clarifications` id（feature 自己 clarify 产生的）+ feature spec 里的 FR/AC/UC id（`featurePlan.spec` 里的，**不含 SpecDecision id**——SpecDecision 是 Clarification 的 spec 层投影，跟随 Clarification replan，不独立持有 status/replacedBy，继承会制造无法作废的引用；对齐 slice 不让 wave 继承 TechDecision 的做法，详见 §5.4）**全量拷贝**到 slice.`basedOnParent`（slice plan 时可以减少到只留自己真正负责的）。**统一存在 `slice.basedOnParent`**，不区分 Clarification 引用和 spec 项引用两套字段（这是 v4 的统一语义，见 epic §7.3）
...
- `abandonItems` 里填要作废的 FR/AC/UC id（epic 层填的是 Clarification id，feature 层填的是 FR/AC/UC id，**机制统一、只是 id 的语义不同**）。**不含 SpecDecision id**——SpecDecision 是 Clarification 的 spec 层投影，跟随 Clarification replan（Clarification 作废时 SpecDecision 自动跟随），不独立持有 status/replacedBy。如果 slice 的 `basedOnParent` 继承了 SpecDecision id，会制造「SpecDecision 无法被 feature replan 作废但又被下游引用」的矛盾（对齐 slice 不让 wave 继承 TechDecision 的做法）。spec 变更走统一 replan 时，只作废 FR/AC/UC（以及 feature 自己的 Clarification）
...
| `SpecDecision`（decisions 章节的元素，`{id, decision, rationale}`）| **SpecDecision 定位为 Clarification 的 spec 层投影**：decisions 章节不独立持有 status/replacedBy，它的内容引用 Clarification（**TODO**：给 SpecDecision 加 `sourceClarification?: string` 字段，记录它投影自哪个 Clarification）。Clarification 被 replan 时，decisions 章节自动跟随（不独立参与 replan）。这样避免「Clarification 能 replan 但 SpecDecision 不能」的不一致。**关键**：SpecDecision id 不被下游 slice 的 `basedOnParent` 继承（slice 只继承 Clarification + FR/AC/UC id）——因为 SpecDecision 不独立持有 status/replacedBy，如果被下游引用会制造「无法被 feature replan 作废但又被 slice 引用」的矛盾。对齐 slice 不让 wave 继承 TechDecision 的做法。|
...
#   注意：不含 SpecDecision id——SpecDecision 是 Clarification 的 spec 层投影，跟随
#         Clarification replan，不独立持有 status/replacedBy，继承会制造无法作废的引用
#         （对齐 slice 不让 wave 继承 TechDecision 的做法，详见 §5.4）
