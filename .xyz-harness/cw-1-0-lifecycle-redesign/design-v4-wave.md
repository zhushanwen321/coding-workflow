...
> 本文档只讲 wave 一层。流程结构与 epic / feature / slice **完全相同**（同样的 8 步、同样的状态机、同样的命令约定），差异只在每步的具体内容——wave 的 plan **写测试代码 + 执行细节**（`WaveTestCase` / `WaveTask` / `WaveFile` / `WaveContract` 集合，TDD 起点 + execute 阶段消费），verify 审查测试覆盖，execute = **dev 写实现代码**（递归出口），test **跑测试**（机器验证，整个 cw 唯一真正的机器 gate）。
>
> **顶层概念体系**（和 epic / feature / slice 一致）：使用者的认知模型只有两个维度——**4 个层**（epic / feature / slice / wave）× **8 个步骤**（create / clarify / plan / verify / execute / test / retrospect / closeout）。每一步在每一层都有一个**同名产物**，需要区分时加层前缀（如 `wavePlan`）。`Clarification`（wave 的）/ `WaveTestCase` 等 wave plan 项都是 wave plan 的**内部字段**，不作为顶层概念暴露。
>
> **前置阅读**：先读 [design-v4-guide.md](./design-v4-guide.md) → [design-v4-epic.md](./design-v4-epic.md) → [design-v4-feature.md](./design-v4-feature.md) → [design-v4-slice.md](./design-v4-slice.md) → 本文档。本文档不重复前 3 层已讲透的内容（4 层总览 / 8 步来源 / execute 递归 / replan 机制 / abandonedRefs / basedOnParent / §8.0 命令约定 / slice 的 plan 项体系 / 基础类型 `WorkUnitItem` / `Plan` / `Split`），只在必要处引用。
>
> **和 epic / feature / slice 机制严格同构**：机制层面（replan、abandonedRefs、reviewedItems、basedOnParent、verifyJudgment / testJudgment / retrospectData）完全一致，差异只在 plan 的内容（wave 是 testCases + tasks + files + contracts，不拆下一层）、execute 的含义（wave = dev 写代码，递归出口）、test 的含义（wave = 跑测试，机器验证）、verify / test / retrospect 的 layerSpecific 字段（偏测试覆盖维度）。学一层，会四层。
>
...
slice 「oauth-backend」                 ← slice 层产物：技术方案（TC/IF/DM/ERR）
  └─ wave 「exchange-token-skeleton」   ← 本文讲这一层
       ...
       ├─ plan:    写测试代码（testCases，TDD 起点）+ tasks/files/contracts（执行细节）
...
| **plan 产物** | 各层方案 + split（拆下一层）| **`wavePlan`：testCases + tasks + files + contracts**（测试 + 执行细节，TDD 起点）|
...
slice 层只产出「技术方案」（plan 项：TC/IF/DM/ERR），粒度是技术选型和接口契约，还停在「怎么设计」层面，没到「怎么实现」。中间缺一层把技术方案落地成可验证的代码：
...
**wave 和 slice 的分工**：slice 管「技术方案合理不合理」（选型靠不谱、接口契约能不能落地、跨 wave 对得上），wave 管「实现 + 测试对不对」（测试覆盖全不全、边界条件想到了没、实现真的让测试过没）。两者不混淆——wave 是纯执行层，不重写技术方案（那是 slice 的事），只照 slice 的 plan 项施工 + 写测试驱动实现。
...
典型 wave 例子（slice = `oauth-backend` 的拆分，承接 slice 文档 §4.3 的 split）：
...
**继承机制**（和 feature / slice 同构，wave **不含 Decision id**）：
- wave 的 `parentUnitId` 指向父 slice
- cw 把父 slice 当前的 `clarifications` 的 id（slice 自己 clarify 产生的）+ slice 技术方案里的 TC/IF/DM/ERR id（**不含 Decision id**——`Decision` 是 Clarification 的技术方案层投影，跟随 Clarification replan，不独立持有 status/replacedBy，继承会制造无法作废的引用；详见 [guide §6.2 跨层规则](./design-v4-guide.md#62-replan-做什么4-步) + slice §4.5）**全量拷贝**到 wave.`basedOnParent`（wave plan 时可以减少——只留自己真正负责的）
- **统一存在 `wave.basedOnParent`**，不区分 Clarification 引用和技术方案项引用两套字段（v4 统一语义，见 epic §7.3）
...
- 「这个函数的异常分支怎么测？」→「每个异常分支独立 WaveTestCase，expected 精确到 error code + message」
...
**和 slice clarify 的区别**：slice 澄清的是**技术细节**（用哪个库、错误态怎么抛），wave 澄清的是**实现细节**（边界条件有哪些、mock 怎么打、测试怎么驱动）。wave clarify 的产物会被 plan 阶段写进 testCases：**`WaveTestCase` 的 `scenario` 字段是 clarify 的直接投影**（边界条件 / 异常分支来自 clarify 的澄清），不需要额外造一个类型（clarify 产物就是 Clarification，plan 时投影成 testCases 的 scenario / 边界条件说明，机制同 slice 的 `Decision` 投影 Clarification）。
...
### 4.3 第 3 步：plan（写测试代码 + 执行细节，TDD 起点）

**做什么**：基于 clarify 的 Clarification + 继承的 slice 技术方案，**写测试代码**（`WaveTestCase` 集合）+ **执行细节**（`WaveTask` / `WaveFile` / `WaveContract`）。这是 wave 层最核心的一步，也是 **TDD 的起点**——先写测试（红），execute 阶段写实现让测试过（绿）。

**wave 的 plan 产物**（4 个字段，都是 plan 阶段产出、execute 阶段消费的「执行细节」，存在 `wavePlan` 里作为内部字段）：

#### wavePlan.testCases（WaveTestCase 集合，TDD 起点）

testCases 是 wave 留给 execute 的「测试驱动契约」。和 slice 的技术方案（`SliceTechChoice` 等）/ feature 的 spec（需求规格）/ epic 的 Plan.split（拆分清单）对应，wave 的核心 plan 产物是 testCases（测试代码）。一个典型的 wave testCases 包含：

| WaveTestCase（例子 wave = `exchange-token-skeleton`）| type | 验证什么 |
|---|---|---|
...
#### wavePlan.tasks（WaveTask 集合，执行步骤清单）

tasks 是 wave 留给 execute 的「执行步骤清单」——execute 阶段照着做。每个 task 记一项执行任务：动什么（impl / refactor / test）、碰哪些文件、按什么步骤、依赖哪些其他 task。

| WaveTask（例子）| type | files | 依赖 |
|---|---|---|---|
| `TASK1: 实现 exchangeToken 骨架` | impl | `src/oauth/token.ts` | — |
| `TASK2: 补 INVALID_CODE 校验` | impl | `src/oauth/token.ts` | `TASK1` |
| `TASK3: 接入 oauth2-client` | impl | `src/oauth/provider.ts` | `TASK1` |

#### wavePlan.files（WaveFile 集合，文件改动清单）

files 是 wave 留给 execute 的「文件改动清单」——execute 阶段照着改。每个 file 记一项文件变更：路径、动作（create / modify / delete）、一句话描述改什么。

| WaveFile（例子）| action | 描述 |
|---|---|---|
| `src/oauth/token.ts` | create | exchangeToken 接口骨架 + TokenPair 返回 |
| `src/oauth/provider.ts` | create | oauth2-client 封装 |
| `src/types/oauth.ts` | modify | 新增 TokenPair 类型定义（承接 slice DM1）|

#### wavePlan.contracts（WaveContract 集合，接口契约）

contracts 是 wave 暴露给**其他 wave 或 slice** 的接口契约——跨 wave / 跨 slice 对齐用。每个 contract 记一项契约：名字、类型（function / api / class）、定义、消费方。

| WaveContract（例子）| type | 定义 | 消费方 |
|---|---|---|---|
| `exchangeToken` | function | `(code: string) => Promise<TokenPair>` | slice `oauth-frontend` 的 wave `login-callback` |

**为什么 wave plan 要有这 4 个字段（调研依据）**：真实项目调研（12 个 topic 的 plan.md）显示 plan 阶段产出大量结构化内容——**Task List 100% 出现**（id / type / files / steps / dependsOn）、**File Structure 83% 出现**、**Dependency Graph 67% 出现**、**Interface Contracts 50% 出现**。v4 把这些归到 wave plan，和 testCases 并列：

| 字段 | 定位 | 阶段角色 |
|---|---|---|
| `testCases` | 测试驱动的契约（TDD 起点）| plan 产出，execute 让它从红变绿，test 机器验全 pass |
| `tasks` | 执行步骤清单 | plan 产出，execute 照着做 |
| `files` | 文件改动清单 | plan 产出，execute 照着改 |
| `contracts` | 跨 wave / 跨 slice 的接口契约 | plan 产出，其他 wave / slice 对齐用 |

它们都是 **wave plan 的内部字段**，不作为顶层概念暴露。每个都继承 `WorkUnitItem`（带 `id` / `status` / `replacedBy`，支持 replan 追踪——某个 task 被废弃，下游 task / wave 能查到，机制见 §5.4）。

**wave 的 plan 项不做强引用 gate**（详见 §5.2）——wave 的 plan 项都是「一个执行单元」的同构结构（不像 slice 的 plan 项有 TC/IF/DM/ERR 四种异构类型），更不需要判别联合，机器 gate 验不出有意义的跨 plan 项约束，靠 agent 自审（`testCaseCoverageNote` 人审判断）。

**关于 split**：wave **不拆下一层**（wave 是叶子，没 childUnitIds）。`WavePlan` 继承 `Plan` 基类所以带 `split` 字段，但 wave 是叶子——`split` 字段对 wave 冗余，**实例化时传 `[]`**。保留这个冗余字段换取 `WorkUnit.plan` 结构全兼容（所有层的 plan 都有 split 字段，统一引擎不用特判 wave）。这是 wave plan 和前 3 层 plan 的本质差异——前 3 层 plan 的 split 拆到下一层，wave 的 split 永远空。

**关于 complexity / goals / background**：feature / slice 有的主观标注、业务目标、背景说明，wave **都不用**——wave 是执行层，背景在 slice / feature / epic 已讲，业务目标是 feature 层的事，主观复杂度评级辅助拆分决策（slice 拆 wave），wave 不再拆。wave 保持 plan 项纯执行驱动（测试 + 步骤 + 文件 + 契约），不和前层的方案 / 需求混淆。

**产出**：`wavePlan`（`testCases` + `tasks` + `files` + `contracts`，外加继承自 `Plan` 的空 `split`）

**状态**：`planning`（progressive，可多次调用，plan 项可多次写）
...
# 输入数据从 stdin 读：
# {
#   testCases: WaveTestCase[],   # 测试代码（存入 wavePlan.testCases）
#   tasks:     WaveTask[],       # 执行步骤清单（存入 wavePlan.tasks）
#   files:     WaveFile[],       # 文件改动清单（存入 wavePlan.files）
#   contracts: WaveContract[]    # 接口契约（存入 wavePlan.contracts）
#   // split 不传入（wave 是叶子，cw 自动填 []）
# }
```

> **wave plan 的状态特化（重要）**：wave 的 plan 允许在 `verified` 状态也能调（详见 §6 状态机说明）。这是 wave 对 slice plan 状态约束（from = clarifying / planning）的**唯一特化点**——因为 wave 是叶子，改 plan 项（testCases / tasks / files / contracts）不影响下游（没 childUnitIds），没有影响面传播，不是 replan 语义。前 3 层 verified 后要改 plan 项走 replan（标下游），wave 没下游可标，所以走 plan progressive 直接改 plan 项（详见 §4.9 wave 特有的改 plan 项机制）。不含 executing 是为避免状态机死锁（详见 §4.9）。

### 4.4 第 4 步：verify（测试覆盖审查）

**做什么**：在 dev 写实现代码**之前**，对 plan 阶段的 testCases / tasks / files / contracts 做**结构化的测试覆盖判断**。机制同 epic / feature / slice verify（见 [epic 文档 §5.4](./design-v4-epic.md#54-第-4-步verify审查规划)）——cw 定义存储结构 + guidance 提示 + 结构校验，**业务判断内容由 agent 产出**。
...
| 维度 | wave 要回答的问题 |
|---|---|
| **必要性（necessity）** | 这个 wave 对父 slice 技术方案的贡献是什么？没它 slice 能交付吗？testCases 驱动的实现是 slice 的哪部分？ |
| **充分性（sufficiency，MECE）** | testCases 加起来覆盖了 slice 负责的所有 IF / DM / ERR 吗？正常路径覆盖了吗？边界条件（空输入 / 超长 / 非法格式）覆盖了吗？异常分支覆盖了吗？有遗漏的测试场景吗？有重复的 WaveTestCase 吗？tasks / files 和 testCases 对得上吗（每个被测分支都有对应的 task / file 改动）？|
| **替代方案（alternatives）** | testCases 有没有过度测试（同一个分支测了三遍）？有没有更高效的覆盖策略（参数化测试 / property-based）？type 选择合理吗（该 integration 的用 unit）？tasks 拆分粒度合理吗（太碎 / 太粗）？|
| **权衡与妥协（tradeoffs）** | 哪些 WaveTestCase 是妥协（如「网络超时不测了留下个 wave」「这个边界条件 mock 太复杂先跳过」）？每个妥协的代价？（留下未测分支的风险）|
...
|---|---|---|
| `testCaseCoverageNote` | testCases 对 slice 技术方案的覆盖度自检（IF / DM / ERR 各覆盖了哪些 WaveTestCase）| 人审判断，机器只验非空 |
...
- **结构完整性**：`test-cases-non-empty`（testCases 至少 1 条）/ `test-cases-have-expected`（每个 WaveTestCase 的 `expected` 非空——这是 TDD 红灯的前提，expected 由 agent 自填断言，cw 不重算）
...
> **关于 tasks / files / contracts 的机器 gate（取舍决策）**：v4 **不**为 tasks / files 加 `tasks-non-empty` / `files-non-empty` 机器 gate，contracts 同理。理由：(1) testCases 是 TDD 红灯的硬前提（没测试就 execute 不了），必须机器验；tasks / files 是「执行细节」，原则上一个极简的 wave（如纯类型定义修正）可以没有显式 task 拆分、只改一个 file、不暴露新 contract——机器强验非空会过度约束 wave 形态。(2) 真实项目调研显示 tasks / files 高频出现（100% / 83%），但「高频」不等于「必填」，contracts 只 50% 出现更不该强制。(3) 机制统一：前 3 层的 plan 项 gate 也只验「核心产物非空」（epic 的 feature-split-non-empty、feature 的 fr-coverage、slice 的 tech-choice-non-empty），不验所有字段。所以 tasks / files / contracts 通过 **guidance 提示**（上面的 sufficiency 维度里问「tasks / files 和 testCases 对得上吗」）让人审盯，不进机器 gate。如果未来发现 agent 经常漏写 tasks / files 导致 execute 卡壳，再升级为机器 gate（YAGNI，guide §3.5）。

**诚实说明**：同 epic / feature / slice——cw 验「agent 有没有填这些字段」，验不了「testCases 写得好不好、覆盖全不全、expected 精确不精确、tasks / files 拆得合理不合理」。内容质量由 agent（或人审）负责。判断存在 `verifyJudgment` 里，是 test / retrospect 对照的基础。

**通过的含义**：testCases 定稿（+ tasks / files / contracts 想清楚了），TDD 红灯就绪，可以进 execute 让 dev 写实现了。
...
早期设计曾考虑「judgeByExpected」机制——cw 从 slice 技术方案契约（IF/DM/ERR）+ WaveTestCase input **重算** expected，对比 actual。**这个机制不成立**，原因：
...
cw 机器验不了的部分（诚实承认）：
- **expected 写得对不对**：WaveTestCase 的 expected 是 agent 自己写的断言，cw 不重算、不验对错——如果 agent 写了错的 expected 让测试「假绿」，cw 验不出（这是 agent 自审 + verify 阶段 `tddRedReadinessNote` 人审的职责）
- **测试覆盖度**（这个 WaveTestCase 集合够不够）：机器验不了，靠 verify 阶段的 `testCaseCoverageNote` / `boundaryConditionNote` 人审
- **测试代码本身的实现质量**（mock 是否失真、断言是否精确）：机器验不了，靠 verify 阶段的 `tddRedReadinessNote` 人审

**manual 类 WaveTestCase 的退化场景**：当 WaveTestCase.type = `manual`（人工验证类，承接 feature AC 的 `verification=manual`），cw 无法自动跑测试——退化为「cw 验 agent 填了 manual 验收记录 + commit 存在」，诚实承认这是机器验证的边界。这种 WaveTestCase 应在 verify 阶段标记 `tddRedReadinessNote` 说明是 manual，test 阶段不强求机器跑。
...
| 复盘维度 | wave 要回答的问题 |
|---|---|
| **判断错误（wrongJudgments）** | verify 阶段哪些测试覆盖判断错了？（标的高风险 mock 实际很稳、判断的 Gap 不存在、认为必要的 WaveTestCase 实际冗余）|
...
| **流程问题（processIssues）** | testCases 写得好吗？（expected 不够精确 / 边界条件漏了 / mock 失真）；实现踩坑了吗？（哪个 WaveTestCase 的实现最难写，为什么）；slice 技术方案够清晰吗？（IF / DM / ERR 模糊导致 expected 重算不出）|
...
**wave 的使命到此结束**——代码已 commit + 测试已过，向上汇总到 slice 的 test 阶段（slice 验 wave 组合起来兑现了 plan 项）。
...
## 5. wave 的 plan 项（`WavePlan` 的组成单元）

### 5.1 wave 的 plan 项是什么

wave 的 plan 项是 `wavePlan` 的组成单元（和 feature 的 SpecSection、slice 的 plan 项对称，**不再是顶层概念**）。它是 wave 把 slice 的技术方案翻译成测试驱动实现 + 执行细节的具体载体。

wave 的 plan 项有 4 种，都是 plan 阶段产出、execute 阶段消费的「执行细节」：

| 类型 | 字段（挂在 WavePlan 上）| 定位 |
|---|---|---|
| `WaveTestCase`（TC）| `testCases[]` | 测试驱动的契约（TDD 起点）|
| `WaveTask`（TASK）| `tasks[]` | 执行步骤清单（execute 照着做）|
| `WaveFile`（FILE）| `files[]` | 文件改动清单（execute 照着改）|
| `WaveContract`（CONTRACT）| `contracts[]` | 跨 wave / 跨 slice 的接口契约（对齐用）|

> id 例子（TC1 / TASK1 / FILE1 / CONTRACT1）保持简洁前缀，类型名加 `Wave` 前缀（避免和 slice 的 `SliceTechChoice` TC1 在跨层引用时混淆，虽然命名空间不同但加前缀更清晰）。
...
WorkUnitItem（基类，定义在 epic 附录）
  ...
  └─ replacedBy?: string
       │
       ├── WaveTestCase     ← extends WorkUnitItem（+ name/scenario/input/expected/type）
       ├── WaveTask         ← extends WorkUnitItem（+ type/files/steps/dependsOn?）
       ├── WaveFile         ← extends WorkUnitItem（+ path/action/description）
       └── WaveContract     ← extends WorkUnitItem（+ name/type/definition/consumers?）
```

4 种 wave plan 项都继承 `WorkUnitItem`，自动带 `id` / `status` / `replacedBy`（支持 replan 追踪：某个 task 被废弃，下游 task / wave 能查到，详见 §5.4）。

**WaveTestCase 的设计选择**（设计决策记录）：

| 设计选择 | wave 采用 | 理由 |
|---|---|---|
| **判别联合 vs 统一结构** | **统一结构**（不判别联合）| slice 的 plan 项用判别联合是因为有 4 种异构类型（TC / IF / DM / ERR）语义不同；wave 的 4 种 plan 项各自字段已经分开存在 `WavePlan` 的 4 个字段里（不混在一个判别联合数组），每个字段内的元素都是同构结构（如 testCases 内都是 `WaveTestCase`），不需要再判别联合 |
| **plan 项间强引用 gate** | **不做**（同 slice 不做 plan 项间强引用 gate）| wave 的 plan 项间没有机器可验的有意义引用关系（一个 WaveTestCase 不引用另一个 WaveTestCase），机器 gate 验不出有价值的约束，靠 agent 自审 |
| **type 字段（WaveTestCase）** | **有**（unit / integration / e2e / manual）| 区分测试类型，影响 execute / test 阶段的执行策略（unit 跑快、integration 需要依赖），机器可验非空 + 集合约束 |
...
interface WaveTask extends WorkUnitItem {
  // 继承自 WorkUnitItem：id（如 "TASK1"）/ status / replacedBy
  type: string;            // "impl" | "refactor" | "test" 等（任务类型，影响 execute 策略）
  files: string[];         // 涉及文件路径列表（对应 WaveFile.path 的子集，agent 自审对齐）
  steps: string[];         // 执行步骤（按顺序做的动作清单）
  dependsOn?: string[];    // 依赖的其他 WaveTask id（task 间 DAG，无循环）
}

interface WaveFile extends WorkUnitItem {
  // 继承自 WorkUnitItem：id（如 "FILE1"）/ status / replacedBy
...
interface WaveContract extends WorkUnitItem {
  // 继承自 WorkUnitItem：id（如 "CONTRACT1"）/ status / replacedBy
  name: string;            // 契约名（如 "exchangeToken"）
  type: string;            // "function" | "api" | "class" 等
  definition: string;      // 契约定义（签名 / schema，自由文本，人审）
  consumers?: string[];    // 消费方（其他 wave / slice 的 id，弱引用——agent 自填，不机器验）
}
```

**字段设计依据**：

- **WaveTestCase.`scenario`**：被测场景描述，含边界条件 / 异常分支说明。**投影自 clarify 的 Clarification**（clarify 澄清的边界条件、mock 策略写进 scenario），不单独造一个类型（设计决策 Q2：wave clarify 产物直接投影进 testCases 的 scenario / 边界条件说明）
- **WaveTestCase.`input` / `expected`**：`expected` 是 **wave 的灵魂字段**。由 agent 自填（断言 / 预期值 / JSON 结构 / 精确字符串 / 正则 / error 对象）。**cw 不重算 expected**——cw 只实跑测试验 pass/fail，expected 的正确性由 agent 自负责（详见 §4.6「为什么 cw 不重算 expected」）
- **WaveTestCase.`type`**：测试类型，影响执行策略。unit（纯函数 / 单模块）/ integration（跨模块 / 需 mock 外部依赖）/ e2e（端到端，需真实依赖）/ **manual**（人工验证，承接 feature AC 的 verification=manual，cw 不自动跑）
- **WaveTask.`type` / `files` / `steps` / `dependsOn`**：执行步骤的结构化记录。`dependsOn` 让 task 间形成 DAG（execute 按 DAG 顺序做），但不做机器 gate 验 DAG（task 间依赖靠 agent 自审，详见 §5.2）
- **WaveFile.`path` / `action`**：文件改动的结构化记录。`path` 独立成字段（不混进 `id`），因为 `id` 是 replan 追踪用的稳定标识，`path` 是物理路径（可能变），两者职责不同
- **WaveContract.`consumers`**：跨 wave / 跨 slice 对齐用。`consumers` 是弱引用（agent 自填，不机器验），影响面查询走 `basedOnParent` 反查，机制同 slice 的 `SliceInterface` 不设强引用 `consumers`（见 slice §5.2）

### 5.2 为什么不做 plan 项间的强引用 gate

slice 层不做 plan 项间强引用 gate（slice §5.2），wave 层同样**不做** plan 项间强引用 gate，理由更强：

- **plan 项间无引用关系**：slice 的 plan 项至少有 `SliceInterface` 引用 `SliceDataModel` 的语义关系（虽然复杂），wave 的 plan 项之间**完全平级**——每个 WaveTestCase / WaveTask / WaveFile / WaveContract 是独立的「一个执行单元」，不引用其他 plan 项（WaveTask.dependsOn 是 task 间顺序依赖，不是引用关系）
- **机器验不出有意义的约束**：plan 项间能验的只有「id 唯一」「type 合法」这种弱约束，靠 agent 自审 + verify 的 `testCaseCoverageNote` / `boundaryConditionNote` 人审判断更合适
- **YAGNI**：强引用 gate 在 wave 层没有解决的问题（覆盖度靠人审，不靠引用关系）

**结论**：v4 wave 不做 plan 项间的强引用 gate，靠人审（测试覆盖 note）。诚实承认这是 agent 自检 + 人审的职责，不假装机器能验。

### 5.3 wave 的 plan 项不是独立工作单元

和 epic 的 Clarification、feature 的 SpecSection、slice 的 plan 项一样，wave 的 plan 项 **只是 wave plan 阶段产生的列表项**，挂在 wave 上：
...
wave
  ...
  ├─ plan: {                        // plan 阶段填（WavePlan，继承 Plan 基类）
  │    split: [],                   // 继承自 Plan：wave 是叶子，永远空 []
...
它没有自己的状态机、不是独立 Unit。wave closeout 后它跟着 wave 一起归档（测试代码进 commit，plan 项记录进 evidence）。

### 5.4 plan 项变更不走 replan，走 plan progressive（wave 特有）

slice / feature 的 plan 项变更走 replan（标下游，影响面传播）。**wave 的 plan 项变更不走 replan**——因为 wave 是叶子（无 childUnitIds），改 plan 项（testCases / tasks / files / contracts）没下游可标，不是 replan 语义（详见 §4.9 wave 特有的改 plan 项机制）。

**wave 改 plan 项的两种路径**：

| 场景 | 路径 | 机制 |
|---|---|---|
| **wave planning 状态时改 plan 项** | plan progressive（多次写） | plan 的正常 progressive 语义，plan 项可多次重写 |
| **wave verified 后改 plan 项** | plan progressive（plan 状态特化）| wave 特化：plan 的 from 加 verified（详见 §4.9）。改完重新 verify 刷新 verifyJudgment。executing 状态需先回 verified（test 还没跑，允许重写 plan 项） |

**plan 项的 `status` / `replacedBy` 字段用在哪**：
- plan 项的 `status=abandoned` + `replacedBy` 用于**记录 plan 项的修改历史**（append-only，保历史，符合 guide §3.6）——agent 改 plan 项时，旧项标 abandoned + replacedBy 指向新版本，而不是直接删除
- 这个历史记录服务于 retrospect（回顾 plan 项怎么演进的）和 evidence（最终交付包含完整的演进轨迹）
- 但**不触发影响面传播**（不标下游、不追加 abandonedRefs）——这是 wave 和 slice / feature 的关键差异：slice / feature 的 plan 项 abandoned 会标下游，wave 的 plan 项 abandoned 只记历史不标下游

**wave 被上游 slice replan 标记时**（这是 wave 的「replan 承受」场景）：slice 改 TC/IF/DM/ERR 触发下游 wave 的 `abandonedRefs`（refKind=`techItem`），wave 被阻塞，调 accept-replan / abort 解锁（机制见 §6）。这是 wave 的 abandonedRefs 唯一来源——**wave 自己改 plan 项不产生 abandonedRefs，只有上游 slice replan 才会给 wave 追加 abandonedRefs**。
...
**已 closeout 的 wave**（边界场景）：slice replan 时如果某个 wave 已经 closed（真终态），cw 静默忽略（不追加 abandonedRefs、不记日志）——机制同 epic §8.2 / slice §6「已 closeout 的下游」段。已交付的 wave 不动，真要跟进新决策由新 wave 重做。
...
#   继承父 slice 的 Clarification id + TC/IF/DM/ERR id（全量拷贝 id 到 basedOnParent，不含 Decision id）
#   注意：wave 是 slice 的直接下游，继承 slice 的 plan 项（TC/IF/DM/ERR），不含 Decision
#         （Decision 跟随 Clarification replan，理由见 guide §6.2 跨层规则）
...
#   #   files:     WaveFile[],
#   #   contracts: WaveContract[]
#   #   // split 不传入（wave 是叶子，cw 自动填 [] 到 WavePlan.split）
#   # }
#   存入 wavePlan（WavePlan 继承 Plan，split 自动填 []）
#   注意：wave plan 状态特化——from 含 verified（不含 executing，详见 §4.9）
#         wave verified 后改 plan 项走 plan progressive，不走 replan
#         executing 状态需先回 verified（test 还没跑，允许重写 plan 项）
...
| **机器验证的实现细节**（cw 如何发现并执行 wave 测试套件、多 commit 场景下选哪个 commit、manual WaveTestCase 的验收记录格式）| 实现侧文档（设计层面本文档 §4.5/§4.6 已讲透机制和能力边界）|
...
interface Wave {
  ...
  basedOnParent: string[];       // 从 slice 继承的 Clarification id + TC/IF/DM/ERR id（plan 时可减少）
                                  // 注意：不含 Decision id（Decision 跟随 Clarification replan，不独立持有 status/replacedBy）
  ...
  abandonedRefs: AbandonedRef[]; // 被废弃的引用及处理状态（和 epic / feature / slice 同构）。
                                  ...
                                  // wave 自己改 plan 项不产生 abandonedRefs（走 plan progressive）
                                  // 空数组 = 不阻塞
  clarifications: Clarification[];          // clarify 阶段填（wave 自己的 Clarification，实现细节维度）
  plan?: WavePlan;                         // plan 阶段填（WavePlan 继承 Plan，含 testCases/tasks/files/contracts 作为内部字段）
...
// WavePlan 继承 Plan（带 split: Split[]，wave 是叶子 split 永远为 []，结构上保留为 WorkUnit.plan 一致性）
interface WavePlan extends Plan {
  // 继承自 Plan：split: Split[]（wave 是叶子，cw 自动填 []，结构兼容 WorkUnit.plan）
  testCases: WaveTestCase[];     // 测试代码（TDD 起点，feature 的 spec / slice 的 plan 项的对应物）
  tasks: WaveTask[];             // 执行步骤清单（execute 照着做）
  files: WaveFile[];             // 文件改动清单（execute 照着改）
  contracts: WaveContract[];     // 跨 wave / 跨 slice 接口契约（对齐用）
}

// ── wave 的 plan 项（4 种，都继承 WorkUnitItem，见 §5.1）──
// 统一结构（不判别联合）：4 种 plan 项各自挂在 WavePlan 的独立字段，字段内同构，不需要再判别联合
...
//   testCaseCoverageNote: string      testCases 对 slice plan 项的覆盖度自检
...
// wave 改 plan 项走 plan progressive（PlanInput），不走 replan
...
const WAVE_VERIFY_GATES = [
  ...
  // 注意：wave 不对 tasks / files / contracts 做机器非空 gate——高频但非强制（详见 §4.4 决策），
  //       也不做 plan 项间的强引用 gate——plan 项间无引用关系，机器验不出有意义约束（见 §5.2）
...
const WAVE_TEST_GATES = [
  ...
  "test-cases-executed",           // 所有非 manual WaveTestCase 都真跑了（机器验测试运行记录，不信任 agent 声明）
  // tests-all-pass（核心机器 gate，整个 cw 唯一真正机器验证业务正确性的 gate）
  // cw 实跑 wave 的测试套件，验所有非 manual WaveTestCase 的测试都 pass（fail 数 = 0）
  // manual 类 WaveTestCase 不机器跑，走退化验证（§4.6）
  "tests-all-pass",
