...
>
> **顶层概念体系**：使用者的认知模型只有两个维度——**4 个层**（epic/feature/slice/wave）× **8 个步骤**（create/clarify/plan/verify/execute/test/retrospect/closeout）。每一步在每一层都有一个**同名产物**，需要区分时加层前缀（如 `epicClarification` / `featureClarification`）。`Clarification` / FR-AC / Split 以及各层 plan 的内部条目类型（feature 的 SpecSection（FR/AC/UC）、slice 的 SliceTechChoice 等、wave 的 WaveTestCase 等）都降级为 clarify/plan 的**内部字段**，不作为顶层概念暴露。
...
| plan | 拆 feature（plan.split）| 写 spec（FR/AC/UC）+ 拆 slice | 拆 wave + 技术方案 | 写测试代码 |
...
interface Clarification extends WorkUnitItem {
  ...
  // status: "active" | "abandoned"——v4 修正：原 cw 0.x 的 "open" | "resolved" | "abandoned"
  // 三值里的 open/resolved 是冗余（resolution 空/非空已表达完成度），v4 统一为两值，
  // 对齐所有 plan item 的 status（都继承 WorkUnitItem）。
  // all-decisions-resolved gate 本来就检查 resolution 字段，不受影响
}
```
...
**产出**：`plan.split`（split：拆 feature 的清单 + 依赖关系图）
...
1. 根据 plan 阶段的 `split` 拆分清单，为每个 feature 创建实例
...
interface Clarification extends WorkUnitItem {
  ...
  // status: "active" | "abandoned"（v4 两值，继承 WorkUnitItem）
}
```
...
epic
  ├─ objective: "..."
  ├─ status: clarifying
  ├─ plan: { split: [...] }          // plan 阶段填（Plan 基类，split 是拆 feature 的清单）
...
| **结构化输入数据** | stdin JSON | split、clarifications、testCases |
...
#   输入数据从 stdin 读：{ split: [...] }
#   含 split（拆 feature 清单，存入 plan.split）
#   多次调用：每次修正拆分方案
...
#   根据 plan.split 创建 feature 实例，每个 feature 进入 created
...
cw epic show <id>                                 # 详情（含 clarifications + plan.split）
```
...
如果被影响的下游已经 closed（真终态，不能改 status），cw **静默忽略**——不追加 `abandonedRefs` 记录（那是阻塞用的，对已 closeout 下游无意义），也不记任何日志（closeout 是真终态不可逆，历史就是历史）。replan 影响面查询算法明确「只查未 closeout 的下游」。如果已交付的下游实现确实因上游变更过期了，由后续新 WorkUnit 引用上游新版本重新做，不回头改已 closeout 的。

> **本设计不引入 staleLog / stale 字段**——cw 0.x 曾考虑过「已 closeout 下游走 staleLog 只记不阻塞」，v4 废弃了这个机制（已 closeout 就是历史，不加任何跟踪字段）。
...
| **输入数据** | split | 要废弃的项 + 新内容（abandonItems / newItems）|
...
4. **split 并入 plan**：拆到下一层不是独立步骤，是 plan 的自然产物（split 是 plan 的内部字段，所有层 plan 共享 `Plan` 基类的 `split` 字段）
5. **顶层概念只两个维度**：4 层 × 8 步产物名。Clarification / SpecSection / FR-AC 降级为各层 clarify/plan 的内部字段，不作为顶层概念暴露（原 DecisionRecord 现已统一为 Clarification，见 v4 二期重构）
...
13. **简单词汇**：字段名用英文（abandoned / replacedBy / abandonedRefs / basedOnParent），叙述用中文大白话（已废弃 / 被替代 / 废弃引用及处理状态 / 引用了父层哪些项）
...
> epic 文档是**机制权威源**——其他层文档引用这里的基础类型。下面先列代码层基础结构（`WorkUnitItem` / `Plan` / `Split`），各层 plan 产物继承 `Plan`、各层可 replan 的条目继承 `WorkUnitItem`。这些是**代码层 DRY 复用，不是领域概念**，不进词表叙述（见 guide §3.5 / §8.4）。

```typescript
// ============ 代码层基础结构（不是领域概念，不进词表）===========
// 所有支持 replan 追踪的条目（Clarification / FR / AC / UC / 各层 plan item）共享此结构
...
interface Epic {
  ...
  plan?: Plan;                              // plan 阶段填（直接用 Plan 基类，只含 split）
...
// ── plan 产物（plan 阶段）──
// epic 直接用 Plan 基类（只含 split），不单独定义 EpicPlan
// split 降级为 plan 的内部字段（顶层概念不暴露 split）
...
interface Clarification extends WorkUnitItem {
  ...
  resolution?: string;                      // 空 = 还没想清楚
  type: "research" | "grilling";
  // v4 修正：原 cw 0.x 的 "open" | "resolved" | "abandoned" 三值里的
  // open/resolved 是冗余（resolution 空/非空已表达完成度），v4 统一为两值
  // 对齐所有 plan item 的 status（都继承 WorkUnitItem）
  // all-decisions-resolved gate 本来就检查 resolution 字段，不受影响
}
...
interface PlanInput {
  split: Split[];   // 存入 plan.split（拆 feature 清单）
}
...
//   slice 能 replan Clarification + FR/AC/UC + TC/IF/DM/ERR（继承的 + slice 自己技术方案项）
...
const EPIC_VERIFY_GATES = [
  ...
  "feature-split-non-empty",          // plan.split 至少一个
