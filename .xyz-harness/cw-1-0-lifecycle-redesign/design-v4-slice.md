...
> 本文档只讲 slice 一层。流程结构与 epic / feature 层**完全相同**（同样的 8 步、同样的状态机、同样的命令约定），差异只在每步的具体内容——slice 的 plan 写**技术方案**（slice 的 plan 项：选型 / 接口 / 数据模型 / 错误处理 / 决策），verify 做结构化技术判断，execute **启动 wave 层**。
>
> **顶层概念体系**（和 epic / feature 一致）：使用者的认知模型只有两个维度——**4 个层**（epic / feature / slice / wave）× **8 个步骤**（create / clarify / plan / verify / execute / test / retrospect / closeout）。每一步在每一层都有一个**同名产物**，需要区分时加层前缀（如 `slicePlan`）。`Clarification` / `FR-AC`（feature 的）和 slice 的 plan 项（`SliceTechChoice` / `SliceInterface` 等）都是各层 plan 的**内部字段**，不作为顶层概念暴露。
>
> **前置阅读**：先读 [design-v4-guide.md](./design-v4-guide.md) → [design-v4-epic.md](./design-v4-epic.md) → [design-v4-feature.md](./design-v4-feature.md) → 本文档。本文档不重复 epic / feature 已讲透的内容（4 层总览 / 8 步来源 / execute 递归 / replan 机制 / abandonedRefs / basedOnParent / §8.0 命令约定 / SpecSection 体系 / 基础类型 `WorkUnitItem` / `Plan` / `Split`），只在必要处引用。
>
> **和 epic / feature 严格同构**：机制层面（replan、abandonedRefs、reviewedItems、basedOnParent、verifyJudgment / testJudgment / retrospectData）完全一致，差异只在 plan 的内容（slice 是技术方案 + split）和 verify / test / retrospect 的 layerSpecific 字段。学一层，会四层。

---

## 1. 一分钟理解 slice 层

slice 是**一个单 session 可完成的技术实施单元**，是 feature 拆出来的、wave 汇聚成的中间层。它把 feature 的「需求 spec」（FR / AC / UC）翻译成 wave 能照着施工的「技术方案」（slice 的 plan 项）。

```
feature 「OAuth 登录」                  ← feature 层产物：需求 spec（FR/AC/UC）
  └─ slice 「oauth-backend」            ← 本文讲这一层
       ├─ clarify: 这个 slice 的技术细节要定清楚，必须先回答什么？（选哪个库、错误态怎么抛）
       ├─ plan:    写技术方案（选型/接口/数据模型/错误/决策）+ 拆 wave
       ├─ verify:  技术方案结构化技术判断（执行前审查）
       ├─ execute: 启动 wave 层（每个 wave 走自己的 8 步，wave 才真正写代码）
       ├─ test:    wave 全完成后，验证组合起来兑现了技术方案
       └─ ...
```
...
| **plan 产物** | `Plan`（基类，含 `split`）+ `clarification.spec`（FR/AC/UC）| **`SlicePlan`（继承 Plan）：技术方案 + `split`（wave 拆分）** |
| **plan 内容性质** | 需求规格（做什么）| **技术方案（怎么做）** |
| **verify 机器 gate** | FR-AC 覆盖 + slice DAG 无环 | **SliceTechChoice 非空 + split 非空 + wave DAG 无环 等**（详见附录 SLICE_VERIFY_GATES）|
...
| **plan 产物** | 技术方案 + `split`（技术方案 + wave 拆分）| `wavePlan`：写测试 + tasks/files/contracts（TDD 红→绿→重构起点 + 执行细节）|
| **是否写代码** | 否（永远不产出代码）| **是**（唯一写代码的层）|
| **粒度** | 单 session 可完成（软锚点 2-4h）| 单次提交可完成（典型 30 分钟 - 2 小时）|

**一句话**：slice 把 feature 的「需求 spec」翻译成 wave 能照着施工的「技术方案」（slice 的 plan 项），自己不写代码。
...
这些「技术细节」就是 slice 的技术方案要回答的问题。slice 层的存在意义：**把 feature 的需求规格，翻译成 wave 可以直接施工的技术方案**。
...
典型 slice 例子（feature = `oauth-login` 的拆分，承接 feature 文档 §4.3 的 split）：
...
- **技术驱动**：slice 的核心产物是技术方案（slice 的 plan 项），下游 wave 照着施工
...
- cw 把父 feature 当前的 `clarifications` 的 id（feature 自己 clarify 产生的）+ feature spec 里的 FR/AC/UC id（`clarification.spec` 里的）**全量拷贝**到 slice.`basedOnParent`（slice plan 时可以减少——只留自己真正负责的）
- **统一存在 `slice.basedOnParent`**，不区分 Clarification 引用和 spec 项引用两套字段（v4 统一语义，见 epic §7.3）
- 这些继承来的引用是 slice 技术方案的初始约束（slice 写技术方案时不能违反它们，不能凭空新增 feature 没声明的功能）
...
**和 feature clarify 的区别**：feature 澄清的是**需求细节**（支持哪些提供商、错误态怎么分），slice 澄清的是**技术细节**（用哪个库、错误态怎么抛）。slice clarify 的产物会被 plan 阶段写进 slice 的 plan 项：**`Decision`** 是 Clarification 的投影（`sourceClarification` 直接引用，机制同 feature 的 Decision），**`SliceTechChoice`** 是独立的选型记录（有 area/choice/alternatives/rationale，不投影 Clarification）。
...
**做什么**：基于 clarify 的 Clarification + 继承的 feature spec，**写技术方案**（slice 的 plan 项集合）+ **拆 wave**。这是 slice 层最核心的一步。

**plan 的两个产物**（都存在 `slicePlan` 里，作为内部字段）：

#### 产物 1：技术方案（`slicePlan.techChoices` / `interfaces` / `dataModels` / `errorSpecs` / `decisions`）

技术方案是 slice 留给下游 wave 的「技术施工图」。v4 把技术方案展开成 `SlicePlan` 的 5 个直接字段（不再用 `techPlan` 判别联合打包），每个字段是一组同构的 plan 项（详见 §5）。一个典型的 slice 技术方案包含：

| slice 的 plan 项类型 | 字段（存在 SlicePlan 上）| 作用 | 例子（oauth-backend）|
|---|---|---|---|
| `SliceTechChoice`（TC）| `techChoices[]` | 技术选型——最核心的技术决策记录 | `TC1: 认证库 = oauth2-client v3.2（备选 passport-oauth / 自研）` |
| `SliceInterface`（IF）| `interfaces[]` | 接口契约——slice 对外 / 对其他 slice 的承诺 | `IF1: exchangeToken(code) => Promise<TokenPair>` |
| `SliceDataModel`（DM）| `dataModels[]` | 数据模型——核心类型定义 | `DM1: TokenPair = { accessToken, refreshToken, expiresAt }` |
| `SliceErrorSpec`（ERR）| `errorSpecs[]` | 错误处理策略 | `ERR1: OAuth 返回 invalid_grant → 返回 401 + 提示重新登录，不重试` |
| `Decision`（D）| `decisions[]` | 技术决策（投影自 Clarification）| `D3: token 存 Redis（投影自 Clarification D3）` |

> `SliceTechChoice` / `SliceInterface` / `SliceDataModel` / `SliceErrorSpec` 都继承 `WorkUnitItem` 基类（带 `id` / `status` / `replacedBy`，支持 replan，详见附录）。`Decision` 是 Clarification 的投影，**不继承 `WorkUnitItem`**（不独立 replan，跟随 Clarification，机制同 feature 的 Decision）。id 例子（TC1 / IF1 / DM1 / ERR1 / D3）保持各自原有缩写——TC/IF/DM/ERR 是 slice 自己的 plan 项（独立继承 WorkUnitItem），D 直接对齐 Clarification 的 id（投影自 Clarification，id 和它一样）。类型名 TC/IF/DM/ERR 加 `Slice` 前缀避免和 wave 的 `WaveContract` / JS 保留字 `Interface` 冲突；Decision 跨层共享不加前缀（feature 和 slice 都用 Decision）。

**slice 不做 plan 项间的强引用 gate**（详见 §5.2）——技术方案间的引用关系（如 `SliceInterface` 引用 `SliceDataModel`、`SliceErrorSpec` 关联 `SliceInterface`）比 feature 的 FR-AC 复杂，机器 gate 验不过来，靠 agent 自审（`interfaceContractNote` 人审判断）。
...
**关于 goals / background / constraints**：feature 有业务目标（goals）和背景约束（background / constraints），slice **不用这些字段**——slice 的背景在 feature / epic 已讲，业务目标是 feature 层的事，slice 是技术层不设业务目标。slice 保持技术方案纯技术，不和 feature 的业务 spec 混淆。

#### 产物 2：wave 拆分清单（`slicePlan.split`）

基于技术方案，把这个 slice 拆成若干 wave（split 在 plan 里做，作为 `slicePlan.split` 内部字段；`split` 字段继承自 `Plan` 基类，slice 用它存 wave 拆分）：
...
> wave 负责哪些 plan 项的引用关系不在 split 里存——wave create 时 cw 把 slice 当前的 TC/IF/DM/ERR id（**不含 Decision id**——`Decision` 是技术决策记录，跟随 Clarification replan，wave 不需引用，对齐 feature 不让 slice 继承 Decision 的做法）全量拷贝到 wave.`basedOnParent`（wave plan 时可减少到只留自己真正负责的），机制见 §4.5。
...
**产出**：`slicePlan`（技术方案字段 `techChoices` / `interfaces` / `dataModels` / `errorSpecs` / `decisions` + `split`）
...
# 输入数据从 stdin 读：
# {
#   techChoices:  SliceTechChoice[],   # 技术选型（存入 slicePlan.techChoices）
#   interfaces:   SliceInterface[],    # 接口契约（存入 slicePlan.interfaces）
#   dataModels:   SliceDataModel[],    # 数据模型（存入 slicePlan.dataModels）
#   errorSpecs:   SliceErrorSpec[],    # 错误处理（存入 slicePlan.errorSpecs）
#   decisions:    Decision[],          # 技术决策（存入 slicePlan.decisions）
#   split:        Split[]              # wave 拆分清单（存入 slicePlan.split）
# }
```

### 4.4 第 4 步：verify（技术方案审查）

**做什么**：在启动 wave 层**之前**，对 plan 阶段的技术方案和 wave 拆分做**结构化的技术判断**。机制同 epic / feature verify（见 [epic 文档 §5.4](./design-v4-epic.md#54-第-4-步verify审查规划)）——cw 定义存储结构 + guidance 提示 + 结构校验，**业务判断内容由 agent 产出**。
...
| **充分性（sufficiency，MECE）** | 技术方案加起来覆盖了 slice 负责的所有 FR/AC 吗？有遗漏的技术点吗？有重叠的 SliceTechChoice 吗？wave 拆分覆盖了所有 slice 的 plan 项吗？ |
| **替代方案（alternatives）** | 技术方案有没有过度设计？选型考虑过其他库 / 方案吗？wave 拆分考虑过其他维度吗？ |
| **权衡与妥协（tradeoffs）** | 哪些 SliceTechChoice 是妥协（如成熟度换性能、便利性换可控）？每个妥协的代价？ |
...
|---|---|---|
| `techChoiceRationale` | 每个 SliceTechChoice 的选型理由是否充分（有没有「随便选了一个」的）| 人审判断，机器只验非空 |
...
> **所有 layerSpecific 字段都是 agent 的人审判断，gate 只验非空，不验内容**。技术方案的一致性自检（如 SliceInterface 引用的 SliceDataModel 是否存在、SliceErrorSpec 是否覆盖 SliceInterface 的错误态）不作为机器 gate——技术方案间的引用关系机器判不准（见 §5.2），那是 agent 自检的职责，放进 `interfaceContractNote` / `crossWaveContractNote` 让 agent 显式回答。

**机器 gate**（同 epic / feature，验结构不验内容）：
- **结构完整性**：`tech-choice-non-empty`（`techChoices` 至少 1 条 SliceTechChoice）/ `split-non-empty` / `split-dag-valid`
...
**诚实说明**：同 epic / feature——cw 验「agent 有没有填这些字段」，验不了「技术方案写得好不好、选型靠不靠谱」。内容质量由 agent（或人审）负责。判断存在 `verifyJudgment` 里，是 test / retrospect 对照的基础。

**通过的含义**：技术方案定稿，可以启动 wave 层了。
...
1. 根据 plan 阶段的 `split` 拆分清单，为每个 wave 创建实例
   - 每个 wave 的 `parentUnitId` 指向这个 slice
   - 每个 wave 的 `basedOnParent` 从 slice 继承：cw 把 slice 当前的 `clarifications` id（slice 自己 clarify 产生的）+ slice 技术方案里的 TC/IF/DM/ERR id（**不含 Decision id**，理由见§5.2）**全量拷贝**到 wave.`basedOnParent`（wave plan 时可以减少到只留自己真正负责的）。**统一存在 `wave.basedOnParent`**，不区分 Clarification 引用和技术方案项引用两套字段（v4 统一语义，见 epic §7.3）
...
**wave 怎么持有 slice 技术方案**（统一引用机制）：

wave 不快照整个技术方案，只记 `basedOnParent`（引用了 slice plan 的哪些项——Clarification id + TC/IF/DM/ERR id 统一存在这一个字段里，不含 Decision id）。如果 slice 改了技术方案（变更 / 撤销某个 SliceTechChoice / SliceInterface / SliceDataModel / SliceErrorSpec），走 **统一 replan**（见 §6）——slice 作废旧项（`status=abandoned` + `replacedBy`），cw 反查子树里 `basedOnParent` 含被废弃 plan 项 id 的 wave，在它们的 `abandonedRefs` 追加记录（refKind=`techItem`），强制处理（accept-replan 或 abort）。机制层面和 epic 改 Clarification、feature 改 FR/AC 完全一致。
...
| 当初判断的技术方案 Gap / 重叠 | 这些 Gap 真的漏了吗？wave 开发时暴露了新 Gap 吗？重叠实际发生了吗？|
...
**诚实说明**：跨 wave 的技术方案兑现验证（如「技术方案里的 IF1 接口真跑通了没、DM1 数据模型真落地了没」）在这里做——单个 wave 只能验自己负责的部分。对照结论由 agent 判断，cw 只验「该对照的都对照了」。

**如果没通过**：可能要回到某个 wave 修，或发现技术方案有问题（少见，verify 已审过；真发生就走 replan）。
...
| 复盘维度 | slice 要回答的问题 |
|---|---|
| **判断错误（wrongJudgments）** | verify 阶段哪些技术方案 / wave 判断错了？（标的高风险选型实际很稳、判断的 Gap 不存在、认为必要的 SliceTechChoice 实现完发现没用）|
| **不良妥协（badTradeoffs）** | 哪些 SliceTechChoice 妥协事后看不值得？（如「选成熟库放弃性能」实际性能瓶颈严重）|
| **遗漏的 Gap（missedGaps）** | verify 没发现、wave 开发才暴露的 Gap？为什么 verify 没发现？|
| **流程问题（processIssues）** | 技术方案写得好吗？（接口契约漏字段 / 选型没考虑兼容性 / 数据模型没想清楚）；wave 拆分合理吗？（太碎 / 太大 / 依赖搞错返工）；技术澄清够吗？（开发一半才发现没定清楚用哪个版本）|
| **提炼经验（lessonsLearned）** | 下次写类似 slice 的技术方案，最该记住的 1-3 条经验？ |
...
**做什么**：
- 写 evidence（slice 的最终交付证据：技术方案最终版、wave 列表、retrospect）
...
## 5. slice 的 plan 项（`SlicePlan` 的组成单元）

### 5.1 slice 的 plan 项是什么

slice 的 plan 项是 `slicePlan` 的组成单元（和 feature 的 SpecSection 对称，**不再是顶层概念**）。它是 slice 把 feature 的需求 spec 翻译成技术方案的具体载体。

**v4 重构：从判别联合展开成 `SlicePlan` 的直接字段**。早期 v4 把技术方案打包成 `techPlan: TechSection[]` 判别联合（一个数组里混 5 种异构类型 + 2 种兜底章节）。v4 重构后，技术方案展开成 `SlicePlan` 的 5 个直接字段（`techChoices` / `interfaces` / `dataModels` / `errorSpecs` / `decisions`），每个字段是一组同构的 plan 项。这样：
- 读 / 写路径更清晰（要看选型直接读 `slicePlan.techChoices`，不用过滤判别联合）
- 类型更精确（每个字段的元素类型固定，TS 能穷尽检查）
- 命名空间隔离（`SliceTechChoice` / `SliceInterface` / `SliceDataModel` / `SliceErrorSpec` 都加 `Slice` 前缀，避免和 wave 的 `WaveContract` / JS 保留字 `Interface` 冲突）

slice 的 plan 项分两类：

**结构化 plan 项**（cw 校验内容 + report 模板渲染）：

| 类型 | 字段（挂在 SlicePlan 上）| 作用 | 对应 feature 的哪个 SpecSection |
|---|---|---|---|
| `SliceTechChoice`（TC）| `techChoices[]` | 技术选型——最核心的技术决策记录 | 对应 feature 的 `functionalRequirements`（FR）：FR 是「要做什么」，TC 是「用什么做」|
| `SliceInterface`（IF）| `interfaces[]` | 接口契约——slice 对外 / 对其他 slice 的承诺 | feature 无对应物（feature 是需求层，不定义接口签名）|
| `SliceDataModel`（DM）| `dataModels[]` | 数据模型——核心类型定义 | feature 无对应物 |
| `SliceErrorSpec`（ERR）| `errorSpecs[]` | 错误处理策略 | feature 无对应物 |
| `Decision`（D）| `decisions[]` | 技术决策（投影自 Clarification）| 对应 feature 的 `decisions`（Decision）——都是 Clarification 的 plan 层投影，跨层共享 Decision 类型 |

> 兜底章节（feature 的 `outOfScope` / `section` / `background` / `constraints` / `goals` / `complexity`）slice 不保留——slice 是技术层，背景在 feature / epic 已讲，业务目标是 feature 的事，主观复杂度评级辅助 slice 拆分决策（slice 拆 wave 靠单次提交判据足够）。真有特殊技术说明写不进上述 5 个字段，写进 `Decision.rationale` 或 `SliceDataModel.notes` 的自由文本即可。

**继承关系**（v4 重构核心）：

```
WorkUnitItem（基类，定义在 epic 附录）
  ├─ id: string
  ├─ status: "active" | "abandoned"
  └─ replacedBy?: string
       │
       ├── SliceTechChoice      ← extends WorkUnitItem（+ area/choice/alternatives/rationale）
       ├── SliceInterface       ← extends WorkUnitItem（+ name/signature/contract）
       ├── SliceDataModel       ← extends WorkUnitItem（+ name/definition/notes）
       └── SliceErrorSpec       ← extends WorkUnitItem（+ interfaceId?/scenario/strategy）

Decision（不继承 WorkUnitItem——投影 Clarification，不独立 replan；跨层共享类型，feature / slice 都用 Decision）
  ├─ id / decision / rationale / sourceClarification?
```

`SliceTechChoice` / `SliceInterface` / `SliceDataModel` / `SliceErrorSpec` 都继承 `WorkUnitItem`，自动带 `id` / `status` / `replacedBy`（支持 replan，详见 §5.4）。`Decision` **不继承 `WorkUnitItem`**——它是 Clarification 的投影，不独立 replan（机制和 feature 的 Decision 完全对称，详见 feature §5.4）。

**设计依据**：slice 的 4 种结构化 plan 项覆盖了技术方案的核心维度（选什么 / 暴露什么接口 / 用什么数据 / 错了怎么办）+ 1 种投影项（为什么这么决定）。slice 是技术层，保持 plan 项纯技术，不和 feature 的业务 spec 混淆。

### 5.2 各类型详解（+ 为什么不做强引用 gate）

各 plan 项类型（除 `Decision` 外）都继承 `WorkUnitItem`，带 `status: "active" | "abandoned"` + `replacedBy?`（和 feature 的 FR/AC/UC 对称，支持 replan，详见 §5.4）。

#### TC（SliceTechChoice）—— 技术选型

最核心的技术决策记录。每个 `SliceTechChoice` 记一项选型决策：选了什么、考虑过什么、为什么。

```typescript
interface SliceTechChoice extends WorkUnitItem {
  // 继承自 WorkUnitItem：id / status / replacedBy
...
#### ERR（SliceErrorSpec）—— 错误处理策略

```typescript
interface SliceErrorSpec extends WorkUnitItem {
  // 继承自 WorkUnitItem：id / status / replacedBy
  interfaceId?: string;    // 关联的 SliceInterface id（可选——全局错误策略可不填，接口级错误必填，让人审能找到错误属于哪个 IF）
...
#### D（Decision）—— 技术决策（投影自 Clarification）

对应 feature 的 `decisions`（Decision）。定位为 Clarification 的技术方案层投影：
...
`Decision` 不继承 `WorkUnitItem`——不独立持有 `status` / `replacedBy`。它的内容引用 Clarification，Clarification 被 replan 时 `Decision` 自动跟随（不独立参与 replan）。机制和 feature 的 Decision 完全对称（详见 feature §5.4）。Decision 是跨层共享类型，feature 的 `clarification.spec` 和 slice 的 `slicePlan.decisions` 都用 Decision（v4 三期统一，原 feature 的 SpecDecision / slice 的 SliceDecision / TechDecision 三个名字合并为 Decision）。`Decision.sourceClarification` 字段记录它投影自哪个 Clarification（id 直接对齐 Clarification 的 id，如 `D3`）。

**Decision 的 `sourceClarification` 指向哪一层的 Clarification**：slice 的 Decision 投影自 **slice 自己 clarify 阶段产生的 Clarification**（存在 `slice.clarifications` 里，不是从父 feature 继承的 Clarification / FR/AC/UC）。理由：
- Decision 是 plan 阶段的产物，只在产生它的那一层有效
- slice clarify 阶段回答的技术问题（如「token 存哪」），答案进 slice 自己的 `clarifications`，同时投影成 slice 的 `decisions` 项（`Decision.sourceClarification` 指向这条 Clarification），供 wave 施工时参考
- slice 从 feature 继承的 Clarification id + FR/AC/UC id（存在 `basedOnParent`，是 feature 层的 plan 项 id）是 slice 的上游约束，不是 slice 自己 clarify 的产物，slice 的 Decision 不投影它们——feature 的 Decision 投影自 feature 自己的 Clarification，是 feature 层的事，和 slice 的 Decision 各自独立
- 同一层的 Clarification id 和 Decision id 一致（如 slice 的 Clarification D5 对应 slice 的 Decision D5），不同层之间 id 独立（slice 的 D5 和 feature / wave 的 D5 是不同 WorkUnit 的不同条目，不冲突）

#### 为什么不做 plan 项间的强引用 gate（Q2）

feature 层做了 FR-AC 强引用 gate（FR 显式声明对应的 AC id），因为 FR-AC 的对应关系简单清晰（一个 FR 对应几条 AC），机器能验「每个 FR 至少有 1 条 AC」。

slice 层**不做**这个 gate，原因：

- **引用关系太复杂**：一个 `SliceInterface` 可能引用多个 `SliceDataModel`（IF1 的返回值用 DM1 + DM2）、多个 `SliceErrorSpec`（IF1 在不同错误场景抛不同 ERR）、被多个 wave 消费。反向关系同样复杂（DM1 可能被多个 IF 引用）。
...
**诚实补充**：有一种低级的机器可验校验被 v4 故意放弃——**id 存在性校验**（如 IF 的 contract 文本里提到 DM5，但 `slicePlan.dataModels` 里没有 DM5）。这种拼写错误级别的检查机器能做、成本极低。v4 选择不做的原因：技术方案中跨项引用主要在自由文本（contract / notes）里，机器要解析自由文本抽 id 既不准又费事，收益有限。这是明确的 v4 取舍，不是「机器完全验不了」。

### 5.3 slice 的 plan 项不是独立工作单元

和 epic 的 Clarification、feature 的 SpecSection 一样，slice 的 plan 项 **只是 slice plan 阶段产生的列表项**，挂在 slice 上：
...
slice
  ...
  ├─ plan: {                        // plan 阶段填（SlicePlan，继承 Plan 基类）
  │    split: [...],                // 继承自 Plan：wave 拆分清单（含依赖 DAG）
...
它没有自己的状态机、不是独立 Unit。slice closeout 后它跟着 slice 一起归档（重要的会被 wave 的 `basedOnParent` 引用，或技术决策类项升级为 ADR）。

### 5.4 技术方案变更走统一 replan

slice 的技术方案不是 immutable——slice 在 `verified` / `executing` 状态都可能改技术方案（wave 开发中发现选型不靠谱、接口契约定错了）。

**统一机制**（和 epic 改 Clarification、feature 改 FR/AC 完全一致）：技术方案变更（修改 / 撤销某个 TC/IF/DM/ERR）走 `cw slice replan`：

- `abandonItems` 里填要作废的 plan 项 id（TC/IF/DM/ERR，**不含 Decision id**——`Decision` 跟随 Clarification replan，不能独立作废）——epic 层填 Clarification id，feature 层填 FR/AC/UC id，slice 层填 slice 的 plan 项 id，**机制统一、只是 id 的语义不同**
- cw 作废旧 plan 项（`status=abandoned` + `replacedBy` 指向新版本）+ 新增新版本（如有 `replacementContent`）
- 反查下游 wave 的 `basedOnParent` 含被废弃 plan 项 id 的，在它们的 `abandonedRefs` 追加记录
- 被标的 wave **阻塞所有改状态 action**（只读查询不阻塞），强制二选一：accept-replan（接受新版本）或 abort（重做）

不引入技术方案快照 / snapshot 双轨机制——统一 replan 已覆盖「上游变 → 下游跟着变」的完整场景。详见 §6 和 [epic 文档 §8.2](./design-v4-epic.md#82-replan局部打补丁影响面传播)。

**注意**：slice plan 项的 replan 通过 `SliceReplanInput.abandonItems` 驱动，`replacementContent` 用 slice 层自己的判别联合（含 TC/IF/DM/ERR 四个 kind，见附录 SliceReplacementContent）。同构的是 replan 的 4 步机制，不是 ReplanInput 的类型集合（详见附录说明）。
...
**已 closeout 的 wave**（边界场景）：slice replan 时如果某个 wave 已经 closed（真终态），cw 静默忽略（不追加 abandonedRefs、不记日志）——机制同 epic §8.2「已 closeout 的下游」段。这是 slice 层最常见的边界场景（一个 slice 拆了 N 个 wave，前 N-1 个 closeout 了，最后一个在跑，这时 slice 发现技术方案要改）。已交付的 wave 不动，真要跟进新决策由新 wave 重做。
...
#   输入数据从 stdin 读：{ techChoices, interfaces, dataModels, errorSpecs, decisions, split }
#   技术方案字段存入 slicePlan.{techChoices,interfaces,dataModels,errorSpecs,decisions}
#   split 存入 slicePlan.split（继承自 Plan 基类）

cw slice verify <id>                               # progressive
#   机器 gate：tech-choice-non-empty / split-non-empty / split-dag-valid
#   业务判断非空（同 epic / feature）

cw slice execute <id>                              # 启动 wave 层
#   根据 slicePlan.split 创建 wave 实例，每个 wave 进入 created
#   每个 wave 的 basedOnParent 继承 slice 的 Clarification id + TC/IF/DM/ERR id（不含 Decision id）
...
#   或 slice 的 plan 项 id（TC/IF/DM/ERR，slice 自己技术方案项）
#   注意：slice 不引入新的 plan 项类型——还是引用 feature 的 Clarification/FR/AC/UC
#         + 自己技术方案项的 TC/IF/DM/ERR id，统一机制
...
cw slice show <id>                                 # 详情（含 slicePlan 的技术方案字段 + split）
```
...
| **机器验证机制**（wave 的测试如何不信任 agent 声明、cw 实跑验全 pass）| wave 文档 |
...
19. **技术方案是 slice 的核心产物，且 IF/DM 稳定性比 TC 还重要**：slice 的价值在于把 feature 的需求 spec 翻译成 wave 能照着施工的技术方案（slice 的 plan 项）。技术方案的 IF/DM（`SliceInterface` / `SliceDataModel`：接口契约/数据模型）一旦不稳，所有 wave 跟着返工——所以 slice plan 在 IF/DM 上花的时间比 TC 还值（TC 选错了只影响一个 wave，IF 不稳会影响所有消费它的 wave）。
...
```typescript
// 基础类型 WorkUnitItem / Plan / Split 定义在 epic 文档附录，本文档不重复声明
// SlicePlan 继承 Plan，slice 的 plan 项（除 Decision 外）继承 WorkUnitItem
// Decision 跨层共享（feature 的 clarification.spec 和 slice 的 slicePlan.decisions 都用 Decision）
...
interface Slice {
  ...
  plan?: SlicePlan;                         // plan 阶段填（含技术方案字段 + split 作为内部字段）
...
// ── plan 产物（plan 阶段）──
// SlicePlan 继承 Plan 基类（含 split: Split[]），扩展 slice 特有的技术方案字段
// v4 重构：从 techPlan 判别联合展开成 5 个直接字段，每个字段是一组同构 plan 项
interface SlicePlan extends Plan {
  // 继承自 Plan：split: Split[]（wave 拆分清单，含依赖 DAG）
  techChoices: SliceTechChoice[];   // 技术选型（clarification.spec 的技术层对应物之一）
  interfaces: SliceInterface[];     // 接口契约
  dataModels: SliceDataModel[];     // 数据模型
  errorSpecs: SliceErrorSpec[];     // 错误处理策略
  decisions: Decision[];            // 技术决策（投影自 Clarification，跨层共享 Decision 类型）
}
// 注意：slice 不再有 techPlan: TechSection[] 判别联合字段（v4 展开为上述 5 个直接字段）
//       不再有 TechSection 联合类型 / WaveSplit 定义（用基类的 Split）

// ── slice 的 plan 项（见 §5）──
// SliceTechChoice / SliceInterface / SliceDataModel / SliceErrorSpec 都继承 WorkUnitItem
// （带 id / status / replacedBy，支持 replan，详见 §5.4）
// Decision 不继承 WorkUnitItem（投影 Clarification，不独立 replan；跨层共享类型 feature / slice 都用）

// 技术选型——最核心的技术决策记录
interface SliceTechChoice extends WorkUnitItem {
  // 继承自 WorkUnitItem：id（如 "TC1"）/ status / replacedBy
  area: string;            // "认证库" / "HTTP 客户端" / "状态管理"
  choice: string;          // "oauth2-client v3.2"
  alternatives: string;    // 考虑过但没选的（"passport-oauth / 自研"）
  rationale: string;       // 为什么选这个（成熟、文档全、社区活跃）
...
interface SliceInterface extends WorkUnitItem {
  // 继承自 WorkUnitItem：id（如 "IF1"）/ status / replacedBy
  name: string;            // "exchangeToken"
  signature: string;       // "(code: string) => Promise<Token>" 或 "POST /api/oauth/token"
  contract: string;        // 输入/输出/错误码/副作用（自由描述，人审）
}
...
// 数据模型——核心类型定义
interface SliceDataModel extends WorkUnitItem {
  // 继承自 WorkUnitItem：id（如 "DM1"）/ status / replacedBy
  name: string;            // "TokenPair" / "users 表"
  definition: string;      // TypeScript 类型定义 / SQL DDL / JSON schema
  notes?: string;          // 约束、索引、不变量说明
}
...
// 错误处理策略
interface SliceErrorSpec extends WorkUnitItem {
  // 继承自 WorkUnitItem：id（如 "ERR1"）/ status / replacedBy
  interfaceId?: string;    // 关联的 SliceInterface id（可选——全局错误策略可不填，接口级错误必填）
  scenario: string;        // "OAuth 提供商返回 invalid_grant"
  strategy: string;        // "返回 401 + 提示重新登录，不重试"
}

// 技术决策（投影自 Clarification）——跨层共享 Decision 类型，对应 feature 的 decisions 章节
interface Decision {
  id: string;                      // 直接用 Clarification 的 id，如 "D3"
  decision: string;
  rationale: string;
  sourceClarification?: string;    // 投影自哪个 Clarification（id 和它一样）
  // 注意：Decision 不继承 WorkUnitItem、不独立持有 status/replacedBy——
  // 引用 Clarification，跟随 Clarification replan（不独立参与 replan）
  // 重要：wave 不继承 Decision id（只继承 TC/IF/DM/ERR），因为 Decision 是决策记录不是施工依赖。
  // 对齐 feature 不让 slice 继承 Decision 的做法（避免「Decision 无法被 slice replan 作废但又被下游引用」的矛盾）
}
...
//   techChoiceRationale: string       每个 SliceTechChoice 的选型理由是否充分
...
interface PlanInput {
  ...
  errorSpecs: SliceErrorSpec[];     // 错误处理（存入 slicePlan.errorSpecs）
  decisions: Decision[];            // 技术决策（存入 slicePlan.decisions）
  split: Split[];                   // wave 拆分清单（存入 slicePlan.split，继承自 Plan 基类）
}

// replan 的输入数据（slice 层定义自己的判别联合，见下方 SliceReplanInput）。
// slice 用 Clarification / FR / AC / UC（继承自 feature 的引用，未减少的部分）+ slice plan 项四种 kind：
//   - Clarification：slice 自己 clarify 产生的
//   - FR/AC/UC：从 feature 继承的 spec 项（slice replan 一般不改这些，但机制上允许）
//   - TC/IF/DM/ERR：slice 自己技术方案项（slice replan 最常见的场景）
...
const SLICE_VERIFY_GATES = [
  // 结构完整性
  "tech-choice-non-empty",         // slicePlan.techChoices 至少 1 条 SliceTechChoice（技术方案的核心）
  "split-non-empty",               // slicePlan.split 至少 1 个（继承自 Plan 基类的字段）
  "split-dag-valid",               // wave 依赖关系无循环
  // 注意：slice 不做 plan 项间的强引用 gate（如 IF→DM）——
