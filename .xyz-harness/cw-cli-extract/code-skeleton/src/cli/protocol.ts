...
/**
 * 编码 cwd 为文件系统安全的目录名。
 * 规则：取绝对路径，替换非字母数字为 `-`，截断到合理长度。
 */
function encodeCwd(cwd: string): string {
  return cwd
    .replace(/^\/+/, "") // 去掉前导 /
    .replace(/[^a-zA-Z0-9]/g, "-") // 非字母数字 → -
    .replace(/-+/g, "-") // 合并连续 -
    .replace(/^-|-$/g, "") // 去掉首尾 -
    .slice(0, 128); // 截断
}
...
export function resolveDbPath(workspacePath: string, cwHome?: string): string {
  ...
  // .cw-wt/ 检测（C-3 worktree cwd 防护）
  if (workspacePath.includes(".cw-wt/")) {
    throw new Error(
...
/** Orphan check wrapper — CwParamsSchema is a const, not a function. */
export function getCwParamsSchema() { return CwParamsSchema; }
