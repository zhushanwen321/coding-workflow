...
npm test            # vitest run（pretest 自动先 build；58 文件 428 用例，M4 修复轮进行中会增长，以实跑为准）
...
- **spec gate 八规则**（`src/gates/spec-rules.ts`，多缺口全列不短路）：① 验收非空；② core 用例自身 type 必须为 e2e 级；③ e2e 用例 command 非空且首 token 在 PATH 可解析；④ e2e-mock 附非空保真说明；⑤ 至少一条 unit 级用例；⑥ split 不得自引用；⑦ 验收 id 字符集（`ACCEPTANCE_ID_RE`，与 e2e-sh marker 同源）；⑧ runner 显式声明必须在 `knownAdapterTypes()` 集合内。验收 type 五枚举：`unit | integration | e2e-real | e2e-mock | manual`。
- **verify 三道 gate**（`src/handlers/verify.ts` + `src/verify/`）：红阶段（测试区分力检查，新测试打到实现前基线树必须挂；默认执行，`--no-red-phase` 逃生口——rv-4 进行中，以交付后实态为准）→ 名字级比对（验收 id 词边界匹配重跑产物用例名，非计数启发式，`src/verify/name-match.ts`）→ 干净 checkout 重跑（账本 commit 检出到一次性工作区 + 独立 CW_HOME，系统自己复跑，`src/verify/checkout.ts` / `src/verify/run.ts`）。exit 语义：`0` 全过；`1` 有 fail（fail 也入账留审计）；`2` 环境错误（不入账）。
...
- **runner 循环**（`src/runner/loop.ts`）：每轮对投影重算 frontier 就绪维度并派发（维度见 `src/readonly/frontier.ts`：specReady / reReview / missingChildren / integrationDrift / integrationReady / buildReady / execReviewReady）；等待 spawn 期间零锁；Ctrl-C 后重跑 `cw run` 从投影续接。每 unit 独立 git worktree + 独立分支（双空间命名 `cw-root/<rootId>` 与 `cw/<rootId>/<unitId>`，`src/runner/worktree.ts`）；spawn 过程产物（brief/stdout/stderr）落 run 级 topic 目录 `~/.cw/topic/<encoded-cwd>/<runTs>-<rootId>/`。内部节点集成 = 确定性代码不派 agent（`src/runner/integrate.ts`：merge 子树 → 干净重跑受影响验收 → 契约两道比对），连续 fail 达上限停止自动重派、转派 designer 处置契约漂移（上限 MAX=1 首败即转——rv-4 进行中，以交付后实态为准）。exec-review 必须携带 `--evidence-refs`，合法集 = 该 unit 已入账 EvidenceSubmitted ∪ VerifyRan 的 runId（`src/handlers/review-submit.ts`）。
- **异源 reviewer 派发**：spec-review verdict 由独立 reviewer spawn 提交（frontier 新增 specReviewPending / specFixPending 维度、reviewer 模型链 `--reviewer-model` / `CW_REVIEWER_MODEL`、spec-review fail 总数 ≥2 转人工）——mx-1 设计 v1.1 已定稿（`docs/rewrite/design-independent-review.md`）、**实现进行中，以交付后实态为准**；当前代码中 spec-review 仍由 designer 任务书自审。
...
| [docs/rewrite/design-independent-review.md](./docs/rewrite/design-independent-review.md) | mx-1 异源 reviewer 派发设计 v1.1（D1-D8，实现进行中） |
