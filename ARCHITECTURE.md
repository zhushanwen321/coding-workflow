...
│  exit code 映射 + 只读查询（tree/status/list/handoff）│
├─────────────────────────────────────────────┤
...
│  15 个 handler，事务内 gate → store 变更      │
├─────────────────────────────────────────────┤
...
| `actions` | 15 个 action handler + CwParams 联合类型 + gateAdvance 深函数 | 新增 action 时加 handler + Params 接口 |
