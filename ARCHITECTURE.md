...
| `core` | `src/core/`（`workunit.ts`/`status.ts`/`plan.ts`/`evidence.ts`/`judgments.ts`/`clarifications.ts`/`git.ts`/`errors.ts`/`frontier.ts`/`hierarchy.ts`/`scheduling.ts`） | 领域模型与类型（零依赖）；`CwError`（预期错误，exit 1）；`frontier.ts` 算 frontier 视图（非终态节点 + blocked/dependsOn）；`hierarchy.ts` 跨父子 WorkUnit 关系只读遍历 + `isDependencySatisfied`（依赖全终态判定，frontier 与 scheduling 复用）；`scheduling.ts` 的 `computeReadyChildren` 算并行就绪批次（伪 Kahn，供 execute/closeout 填 parallelTargets，详见 [ADR-0011](./docs/adr/0011-recursive-parallel-scheduling.md)） | 核心契约，变更影响面大 |
| `guidance` | `src/guidance/`（`build-guidance.ts`/`cross-layer.ts`/`failure-hint.ts`/`prefix-builder.ts`/`schema-injector.ts`/`subagent-guidance.ts`/`templates/`） | 拼入 `nextAction.guidance` 的纯文本提示词（正常三段式 / 异常四段式，并行场景额外渲染「## 并行调度」段），agent 的唯一导航来源；`cross-layer.ts` 算 closeout 回溯方向（sibling/ascend/undefined）+ `computeParallelSiblingsAfterCloseout`（就绪兄弟并行批次）；schema 段取 **nextAction**（非刚完成 action）；`prefix-builder.ts` 渲染剥离 layer: 前缀（单 layer 显示）；replan guidance 透传 schema 段 | 阶段方法论变化时改 template | [from: cw-guidance-hardening §system-architecture] |
...
返回类型 `ActionResult`（`src/handlers/types.ts`）：`unitId` / `status` / `ok` / `gateResults?` / `error?` / `replanImpact?` / `freezeViolations?` / `failureCount?` / `children?`（execute 下沉时返回子层信息 `{unitId, dependsOn}[]`，供递归调度器消费）/ `nextAction?`（含 `parallelTargets?`：execute 下沉 / closeout 回溯时填的就绪子 unit 并行批次，详见 [ADR-0011](./docs/adr/0011-recursive-parallel-scheduling.md)）。依赖注入接口 `CwDeps`：`store` / `gitValidator` / `testRunner?` / `fileExists` / `workspacePath` / `clock`——所有 IO 能力通过此接口注入，handler 本身不直接做 IO。
...
| `retrospect` | wave 2 个 + PlanningUnit 额外多个 | wave: `lessons-learned-non-empty` + `retrospect-covers-judgments`；PlanningUnit 额外: `all-waves-closed`/`split-fulfillment-covers-plan`/`reviewed-items-cover-design-review`/`child-unit-evidence-complete`/`delivery-verdict-non-empty`/`child-delivery-consistency`/`slice-lessons-learned-non-empty`（含 `deliveryVerdict` 验收子层） | `src/rules/gates/retrospect.ts` |
