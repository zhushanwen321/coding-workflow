...
6. **失败打回**：原会话定向修（贴上 diff/失败输出/违反的条款）。轮次 +1；超 2 轮未绿 → 冻结该单元，升级用户（附已尝试方案清单）
7. **波间推进**：整波 committed 才开下一波
8. **循环至状态表全 committed** → 转入 `flow/consistency-review.md`

## worktree 单元的差异

派发 cwd = 该 worktree 绝对目录；核验、commit 都在该 worktree 内由主 agent 完成；合并回工作分支的操作与时机遵循项目既有约定。集成性质的下游单元必须在合并完成后才能开工。
