...
| 功能域 | 成熟度 | 说明 |
|--------|--------|------|
| **状态机编排** | 核心 | 16 action / 两层 status（PlanningStatus 8 + ExecutionStatus 10）/ 单层 guard（`guardWave`/`guardPlanning` 防跳步，fail 抛 `illegal_transition`）[from: CONTEXT.md「两层 Status」「状态流转规则」] |
