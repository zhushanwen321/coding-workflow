...
describe("u6c pi 适配器：真实 pi 微调用 E2E", () => {
  // real-pi 守卫（tests/fixtures/pi-env.ts）：CI 一律 skip；本地要求 node_modules
  // 外的真实 pi——vendored 副本（node_modules/.bin/pi）不算
  const itRealPi = hasRealPi ? it : it.skip;

  if (!hasRealPi) {
    console.warn("u6c: 无可用真实 pi（CI 环境或 PATH 仅有 vendored 副本），真实 E2E 条跳过");
  }
