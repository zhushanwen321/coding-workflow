...
describe("E11a: plan gate fail（format 非 lite）→ status 仍 created, retry", () => {
  it("format=mid-clarify → status=created, nextAction=plan", () => {
    ...
    // FR-1: plan 前必须先 clarify → confirm_clarify，否则 illegal_transition 抢在 format gate 前。
    setupToClarifyConfirmed(e, "e11a-fail", topicId);
    const result = parseStdout(
    ...
    // status 不变（仍 clarify_confirmed，format gate fail 不流转）
...
describe("E11b: 连续 5 次 plan gate fail → guidance 含熔断", () => {
  it("第 5 次 fail 后 guidance 触发熔断文案", () => {
    ...
    // FR-1: plan 前必须先 clarify → confirm_clarify。
    setupToClarifyConfirmed(e, "e11b-circuit", topicId);
...
describe("E11c: fail 1 次后重试成功 → status=planned", () => {
  it("先 fail（format 错）再 pass（format=lite）→ planned", () => {
    ...
    // FR-1: plan 前必须先 clarify → confirm_clarify。
    setupToClarifyConfirmed(e, "e11c-retry", topicId);
    ...
    expect(failResult.status).toBe("clarify_confirmed");
