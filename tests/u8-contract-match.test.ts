 ...
 * rv-4 语义迁移：契约输入结构带 owner（ContractMatchInput.contracts 改
 * OwnedContract[]，废除同 id root 优先去重）；本文件锁定的树内验证语义不传
 * frozenByUnit（配对第一道由 tests/rv4-contract-pairing.test.ts 专项覆盖）。
 * 用例编号「验收N」逐条对应验收文档：
...
describe("验收2：契约含 file → 未命中 → 失败含契约 id、文件路径与恢复动作", () => {
  it("signature 不在期望文件 / 期望文件不存在 → 两种未命中都指明 C1 与路径", () => {
    const miss = matchContracts({
      contracts: [owned(contract({ signature: "export function capitalise(", file: "src/capitalize.js" }))],
    ...
    const absent = matchContracts({
      contracts: [owned(contract({ file: "src/nope.js" }))],
...
describe("验收4：契约缺 file → 全树未命中 → 失败含契约 id 与恢复动作", () => {
  it("树内无任何文件含签名 → ok=false，failure 指明 C1", () => {
    ...
    const result = matchContracts({
      contracts: [owned(contract({ signature: "export function capitalize(" }))],
      checkoutDir: tree,
    });
...
describe("验收5：全树搜索跳过 node_modules 与二进制文件", () => {
  it("签名只存在于 node_modules 诱饵与二进制文件中 → 仍判未命中（跳过即不产生假阳性）", () => {
    ...
    const control = matchContracts({ contracts: [owned(contract())], checkoutDir: cleanTree });
...
describe("验收6：多契约独立判定不短路；空契约列表 → ok=true", () => {
  it("一挂一带 → failures 恰一条且只指向未命中契约；空列表 → ok", () => {
    ...
    const result = matchContracts({
      contracts: [
        owned(contract({ id: "C1", signature: "export function capitalize(", file: "src/capitalize.js" })),
        owned(contract({ id: "C2", signature: "export function sluggify(", file: "src/sluggify.js" })),
      ],
      checkoutDir: tree,
    });
    expect(result.ok).toBe(false);
