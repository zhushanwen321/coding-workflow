...
describe("assertSafeSize (via parseLitePlan)", () => {
  it("超大 plan（> 1MB）→ throw CwError 含 too large", () => {
    ...
    const plan = {
      format: "lite",
      objective: "test",
      waves: [{ id: "W1", changes: [{ file: "src/app.ts", action: "create", description: bigText }], dependsOn: [] }],
      testCases: [
        {
          id: "U1",
          layer: "mock",
  ...
  it("正常大小 plan → 不 throw", () => {
    const plan = {
      format: "lite",
      objective: "test",
      waves: [{ id: "W1", changes: [{ file: "src/app.ts", action: "create", description: "改 src/app.ts" }], dependsOn: [] }],
      testCases: [
        {
          id: "U1",
          layer: "mock",
