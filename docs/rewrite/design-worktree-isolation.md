...
（fx-4 现状注，2026-08-18：上图 worktree 内的 `.cw-spawn/…{brief.md,stdout,stderr}` 已随 fx-4 迁出——spawn 过程产物现落 run 级 topic 目录 `<CW_HOME>/topic/<encoded-cwd>/<runTs>-<rootId>/`，worktree 内只承载 agent 业务产出与 commit；上图与 human 指引示例中的 `.cw-spawn/` 路径为 v3 时代口径。）
...
**D4 reset 语义回归 canon：删近似实现，对 unit worktree `reset --hard` + 裸 `clean -fd`（fx-4 纯化后形态，2026-08-18 回写）。** 每次派发前（重派或换角色）对该 unit 的 worktree 执行 `git reset --hard HEAD && git clean -fd`：清掉未提交半成品（含 untracked），保留已 commit 产出。**fx-4 产物迁移后无任何 `-e` 例外条款**——spawn 过程产物（brief/stdout/stderr）已迁 run 级 topic 目录（`<CW_HOME>/topic/<encoded-cwd>/<runTs>-<rootId>/`），worktree 内不存在任何 cw 想保护的东西；agent 自建的 `.cw-spawn/` 等目录属普通 untracked，被清是正确语义（`src/runner/worktree.ts` resetWorktree 裸 `clean -fd`，注释明示「无任何 -e 例外条款」）。（历史旁注：v3 时代的 `-e .cw-spawn` 排除语义已随 fx-4 反转——彼时产物落在 worktree 内 `.cw-spawn/`，裸 `clean -fd` 会把换角色派发时上一角色的产物整个删掉，故 v2 要求 `-e .cw-spawn`（探针 P-wt8）；产物外迁后该权衡消失，`-e` 从「必须」变为「不存在」。）closed unit 回收时产物不随 worktree 销毁——stdout/stderr/brief 在 topic 目录永久保留（跨 run 历史按 runTs 目录名可查，D5），审计价值不依赖 worktree 现场；debug 需求最高的失败/转人工场景仍不回收 worktree（D5）。`clean -fd` 是安全的——worktree 由 cw 创建管理，cw 自身文件已尽数外迁，目录内不存在认知外保护对象（§2.3 失败模式 C 的权衡消失）。`checkWorkspaceForDispatch`（`loop.ts:799-826`）及其调用点（960）整体删除；「reviewer 在跑时不动 builder 产出」由「同 unit 同时只有一个 agent」的派发不变量天然保证，无需 in-flight 分支逻辑。
...
|---|---|---|
| 在 | 在 | 复用：`reset --hard` + 裸 `clean -fd`（fx-4 后无 `-e .cw-spawn`，见 D4 反转注）后续派发（跨 run 常态，零人工） |
...
root worktree 适用同一矩阵：集成 merge 前确保存在（防御性——root spec 冻结后不再派 designer、无首派创建时机，worktree 若亡则按「亡/在」自动重建；正常时序 root 最后 closed，其 worktree 全程在）。回收（fx-5 成对回收）：unit closed 后不立即删（debug 常需翻看现场），下一轮 runner 循环统一回收上一轮 closed 的 unit 资源——worktree 目录（`git worktree remove --force`——探针 P-wt4：脏残留需 force）+ 子分支成对；失败/转人工的一律保留并在指引中打印路径（fx-4 后指引打印 run 级 topic 产物路径——stdout/stderr/brief 在 `<CW_HOME>/topic/<encoded-cwd>/<runTs>-<rootId>/` 下，worktree 现场保留语义不变——2026-08-18 回写注）。**启动孤儿清扫**（回收的跨进程兜底——「上一轮 closed」的内存态判断跨 run 不可靠）：runLoop 启动时双扫——worktree 目录（`~/.cw-worktrees/<encoded-cwd>/` 下全部）+ 子分支 ref（`refs/heads/cw/` 命名空间，`cw-root/` 前缀不在其中、成果分支天然不被扫出），unit 已 closed（或账本内不存在该 unit）→ 成对回收，防反复中断导致的资源堆积（含「目录已亡分支残留」的孤儿分支形态）。注意与并行 run 的边界：同项目另一 run 的未 closed unit 不在账本本 root 子树内，按「账本内存在且未 closed」判断保留（跨 root 并行时扫的是同一 `<encoded-cwd>` 目录，判定必须查全账本而非本 root 子树）。进程数边界：同一 root 同时只允许一个 runLoop 进程——双进程并行会对同 unit 双派发到同一 worktree、退回共享工作区混卷（M1 共享 cwd 时代同样如此，非本设计引入的回归）；跨 root 并行无此约束（受账本级 unitId 唯一性约束，见 D2）。子分支回收谓词（fx-5）：unit 终态（closed 或账本无主）且分支 tip 经 root 分支可达（探针 P-wt5：GC 安全）→ 删；tip 不可达或 root 分支不存在 → 保守保留并出声（分支是未回流产出的唯一 ref 锚点，指引手动确认后 `git branch -D`）。merge 点本身不删分支（v3.2 勘误：原「merge 进 root 分支后可删」由 merge 成功路径内联执行，但「冲突→人工解→集成重跑」路径上子 commit 已可达、走步骤 0 的已达跳过，永久绕过删除造成分支残留——M3 gate 两次复现；删除统一由终态谓词承担，与执行路径无关）；root 分支 run 结束后保留供回流（G5），不在自动回收范围。root closed 汇总输出打印回收清单（§3.1 样例末段）。
...
| P-wt8 | `git clean -fd -e .cw-spawn` 精确排除产物目录 | 实测：含 `.cw-spawn/u.stdout`、`node_modules/`、untracked 文件的 worktree 执行 → 其余全删、`.cw-spawn/` 保留 | ✅（v3 口径 D4 排除参数的依据；fx-4 产物迁 topic 后 `-e` 语义已失效，见 D4 反转注） |
...
| 2 | **半成品清理回归**（G2） | builder 被人为注入失败（exit≠0），在其 worktree 留 tracked 脏改 + untracked 构建产物；触发重派 | 重派的 agent 开工前该 worktree `git status --porcelain` 全空（fx-4 后裸 `clean -fd`，`.cw-spawn/` 伪造物一并被清——产物已迁 run 级 topic 目录，worktree 内无例外；v3 时代「除 `?? .cw-spawn/` 外为空」口径随 fx-4 反转）；既有 u7b 系测试改造后转绿 |
...
| **W3 reset 语义替换** | 删 `checkWorkspaceForDispatch`（loop.ts:799-826 + 960 调用点），派发前改调 worktree reset+clean（`-e .cw-spawn`，W2 矩阵「在/在」分支已内聚；fx-4 已反转为裸 `clean -fd`，见 D4） | D4；必须在 W2 之后（worktree 存在才有 reset 对象） | 场景 2（半成品清理） |
...
- 2026-08-18 v3.2（fx-5 成对回收）：D5 分支清理语义重写——merge 成功路径的内联 `branch -D` 删除（「冲突→人工解→重跑」路径上已达跳过永久绕过它，M3 gate 现场两次复现分支残留；且「分支已删+worktree 仍在」中间态会撞 D5 矩阵「在/亡」异常格，阻断「在其 unit 重新提交 build 证据」的恢复路径）；回收统一为状态驱动成对回收——单一谓词（unit 终态 × 分支 tip 经 root 分支可达，不可达保守保留+出声）、单一 owner（worktree.ts reclaimUnit）、双道机制扩为目录+ref 双扫。merge 点自此无资源回收副作用。
- 2026-08-18 v3.3 回写：`-e .cw-spawn` 全部语义随 fx-4 反转为裸 `clean -fd`（spawn 产物迁 run 级 topic 目录 `<CW_HOME>/topic/<encoded-cwd>/<runTs>-<rootId>/`，worktree 内无 cw 文件，无例外条款——D4/场景 2/W1 注记/W3/文件改动地图同步）；指引路径口径随 fx-4 迁 topic（失败/转人工指引打印 stdout/stderr/brief 的 topic 路径，worktree 现场保留语义不变——D5）——来源 2026-08-18 实现-设计对抗审查。
