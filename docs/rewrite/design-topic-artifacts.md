...
- **C（冲突）**：`.cw-spawn/` 是 untracked，agent 的 `git add -A`（brief 任务书教的形态）会把它卷进 commit——M3 终验现场：`cw-root/md-reader` 分支含 6 个 `.cw-spawn` 产物文件，用户回流主分支时会带出这些文件。flaky 探针还展示了第二形态：`clean -fd` 删掉真产出的同时 `add -A` 卷入产物（双害同源）。
...
- **G3' 审计自包含**：spawn 日志按 run 归档永久保留（与 evidence 同级审计资产）；spec 原文（此前只存账本 hash）copy 进 evidence，证据链可重读原文。
- **G4' 零回归**：账本、evidence 布局、CW_PROJECT_DIR 锚定、human 转人工路径、verify 语义全部不变。

### 1.2 scope

- **in scope**：spawn 产物落点迁移（topic 目录 + artifactDir 契约）、`clean -e` 补偿删除、spec 原文 copy 进 evidence、场景 4 反向断言补测试、worktree.ts 头注释 v2 旧口径修正（偏离审查 #3）。
- **out of scope**：账本 schema 与事件类型（零变更）；`~/.cw/<encoded-cwd>/events.log` 与 `evidence/` 布局（不变）；`~/.cw-worktrees/` 布局与回收语义（不变）；`/tmp` 的 verify mkdtemp（用后即清，不需收口）；topic 目录的自动清扫（永久保留，P2 拍板——不引入新清扫复杂度）。
...
~/.cw/topic/<encoded-cwd>/<runTs>-<rootId>/
        <unitId>.<role>.brief.md / .stdout / .stderr   ——spawn 过程产物（新）
~/.cw/<encoded-cwd>/events.log          ——不变
~/.cw/<encoded-cwd>/evidence/...        ——不变（+ spec 原文副本，D4）
/tmp/cw-verify-*                        ——不变
```
...
**C 的失败形态**（若采用）：终验现场 builder 用了 `git add -A`（任务书模板示例的形态）——提示词层约束在真实 agent 行为面前是概率性的，与「机器检查 gate」的产品哲学相悖。

### 3.3 关键决策

**D1 topic 路径：`~/.cw/topic/<encoded-cwd>/<runTs>-<rootId>/`（P1 拍板：带 encoded 层）。** `<encoded-cwd>` 与账本/worktree 同 key（归属排查一眼对应，跨项目零歧义）；`<runTs>` 格式 `YYYYMMDD-HHmmss`（runLoop 启动时刻本地时间）；`<rootId>` 即 slug。一次 run 一个 topic 目录：同 run 重派沿用 append（「历次输出不覆盖」承诺不变）；跨 run 重跑 = 新目录（按 run 归档，优于现状的跨 run 混 append）。位于 CW_HOME 内，测试隔离沿用 `CW_HOME` 覆盖，**不新增 env**。

**D2 spawn 契约：`AgentSpawnRequest` 加必填 `artifactDir: string`。** runner 在派发点传 topic 内该 unit+role 的目录路径（`<topicDir>/<unitId>/<role>/`——按 unit+role 分子目录，替代现有文件名前缀拼接；重派 append 语义由 lifecycle/human 的 "a" flag 延续）；适配器（pi/human）只从 `req.artifactDir` 拼 brief/stdout/stderr 文件名——适配器不再感知产物布局，lifecycle 零改动（本就只管传入路径）。`workdir` 字段语义不变（纯 agent 工作区）。
...
**D4 spec 原文入 evidence（P3 拍板：顺手修）。** `evidence submit --kind spec` 在现有 sha256 记账之外，把 spec.json 原文 copy 进 `evidence/<unitId>/spec/<specHash>.json`（以 specHash 命名天然幂等去重——同 spec 重复提交零增长）。build 形态不 copy（`--file` 本体本就在 commit 树里，commit 可达即审计可读）。账本零变更（paths/sha256 字段不动，副本是纯增量审计资产）。

**D5 生命周期：topic 目录永久保留（P2 拍板）。** 与 evidence 同级审计资产；每次 run 量级 MB 以下，堆积可忽略；不引入自动清扫（避免「回收清单」机制再扩一层复杂度——worktree 回收有 debug 窗口权衡，topic 日志没有「现场被破坏」问题，保留无损）。人工清理口径：整个 topic 目录删除不影响账本重放（账本不引用 topic 路径——`SpawnResult.stdoutPath` 只进运行时返回值与 stderr 提示文案，不入账本事件）。

**D6 转人工指引跟随。** human 指令的 `cat "<briefPath>"` 自动跟随（briefPath 指向 topic 内绝对路径）；「干活 cd worktree、提交 CW_PROJECT_DIR 前缀」双路径结构不变；escalation/idle 文案的 stdout 路径跟随 topic。
...
| 3 | **产物可达与归档（G3'/审计）** | 同 run 内同 unit 重派两次 → 跨 run 再跑一次 | topic 目录内 stdout 为历次 append（两次内容都在）；第二次 run 落**新** topic 目录（runTs 不同）；spec 提交后 evidence 下存在 `<specHash>.json` 副本且与原文逐字节一致 |
...
| 落点 | `store/project.ts` +topicDir/runArtifactDir 路径函数；`loop.ts`（runLoop 启动建 topic、writeBriefFile、派发点传 artifactDir、escalation/idle 文案）；`pi.ts`/`human.ts` artifactPaths 改源 |
| 纯化 | `worktree.ts` resetWorktree 删 `-e`（+头注释 v2 旧口径修正，偏离审查 #3） |
| spec 副本 | `handlers/evidence-submit.ts` submitSpec copy `<specHash>.json` |
| 测试 | topic 布局/append/跨 run 三组新测试 + 场景 1-4 条款 + 既有 `.cw-spawn` 断言迁移（wt2/wt3/wt4/u7 系等，估 ~30 处）+ 场景 4 反向断言 |
| 波外不动 | integrate.ts、verify/、core/、events/、cli.ts、账本/evidence 布局 |

**待验证检查点（实施期）**：① topic 目录名在秒级并发双 run 下的碰撞面（同 rootId 并行被 D5 单进程口径禁止，不同 rootId 同秒 = 不同目录名，无碰撞）；② human 指引中 topic 绝对路径在含空格项目路径下的引号形态（复用 wt-2 双引号规则，测试覆盖）。
