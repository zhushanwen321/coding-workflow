...
| mx-4 | spec 打回代数预算放宽（默认 2→10 + --max-spec-rejects 可配置） | building | <本 commit> | 用户 2026-08-19 裁决（gate 二跑后）：reviewer 两代真意见非活锁被误停，默认支持 10 轮；暂不收官。只动阈值与配置面，代数语义/flake/集成上限锁定零变更 |
