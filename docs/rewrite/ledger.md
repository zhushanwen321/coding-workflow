...
| fx-2 | 集成层死锁 R4 修复 | building | 待基线 | 终验第 2 次 FAIL：R4a 契约漂移（async 一字节差）无恢复出口 + R4b 集成审计事件致 idle 永不触发（31 轮死循环）。fx-1 三修复现场全部生效（R2 105s 恢复 vs 首次 45min 死锁；两叶首次全 closed；产物全绿） |
