...
| doc-3 | parent/spawn/testrun/wt 设计文档过时注回写 + fx-5 验收链补录 + fx-3 ledger 修复 + P1 探针 + 回收审计二跑 | building | — | 审查 A-3/A-4/A-5/D1/D5/D10/D12。已完成：设计文档回写（5 文件，事件 140）、fx-3 ledger 结构修复（事件 138）、fx-5 验收链补录（fx5-report.md）、**回收审计二跑 PASS**（recycle-audit-2-report.md：8 基线 diff/sha256/报告账本对应/重跑全过 + 定型态 450 绿；5 发现项移交——2 勘误已由主 agent 例外备案修订、3 移交项见事件流水）。剩余：P1 探针（fold 性能） |
