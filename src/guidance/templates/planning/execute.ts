...
export const PLANNING_EXECUTE_TEMPLATE: PlanningStageTemplate = {
  ...
  dispatchGuidance:
    "execute 已创建 N 个 child，派 N 个子层 agent 并行推进（每个 agent 一个 child；派哪个 agent 见上派发段，task 见上）后空闲等 steer 唤醒；唤醒后 cw status --unitId <本单元> 复查：子全完（closed）则派 chain-agent 合并子交付 + 推进本层 retrospect；未完则继续等，不自己 descend。",
};
