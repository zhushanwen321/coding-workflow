...
export const PLANNING_EXECUTE_TEMPLATE: PlanningStageTemplate = {
  ...
  // G1 + G5：recursive 模式续 turn 指导——父 execute 后派 N 个 wave-agent 并行，空闲等唤醒，
  // 唤醒后查 status：子全完则派 chain-agent 合并 + 推进本层 retrospect，未完继续等。
  dispatchGuidance:
    "execute 已创建 N 个 child，派 N 个 wave-agent 并行推进（每个 agent 一个 child，task 见上）后空闲等 steer 唤醒；唤醒后 cw status --unitId <本单元> 复查：子全完（closed）则派 chain-agent 合并子交付 + 推进本层 retrospect；未完则继续等，不自己 descend。",
};
