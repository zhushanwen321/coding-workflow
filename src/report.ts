...
function renderReviewIssues(topic: Topic): string {
  ...
  const rows = topic.reviewIssues
    .map((issue) => {
      ...
      return `        <tr><td><code>${esc(issue.id)}</code></td><td><span class="badge ${sevClass}">${esc(issue.severity)}</span></td><td>${esc(issue.description)}</td><td>${issue.ref ? `<code>${esc(issue.ref)}</code>` : "—"}</td><td>${status}</td></tr>`;
    })
    .join("\n");
...
${renderDocArtifact("Review Report", safeDocs.reviewDoc, topic.artifacts?.review?.path, topic.artifacts?.review?.at)}
${renderRetrospect(topic)}
${renderDocArtifact("Retrospect Report", safeDocs.retrospectDoc, topic.artifacts?.retrospect?.path, topic.artifacts?.retrospect?.at)}
  <div class="footer">
