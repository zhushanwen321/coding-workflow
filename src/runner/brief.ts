...
function specReviewReviewerTasks(
  ...
  return [
    ...
    "      - unit / integration 型逐条问：命令是否 vitest 兼容——--reporter 若出现，gate",
    "        规则⑨口径更严：仅等号形态 --reporter=json（值恰为 json）放行；空格形态",
    "        --reporter json 无论值一律拒——cw 自动追加 --reporter=json，命令自带其他",
    "        reporter 形态/值会让 stdout 变成文本+JSON 混合体、解析必挂；install 步骤带 --silent。",
