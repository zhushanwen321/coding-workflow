 ...
 *
 * 设计要点：
 *   - 只接收 CwStore / WorkUnitRecord 数据 + 参数，不触碰文件系统、不写 store。
 ...
 * @param store       CwStore（只调 load / findChildren 读方法）
 */
export function renderTree(rootUnitId: string, store: CwStore): string {
...
/** renderHandoff 需要的 store 读方法结构类型（与 renderTree 同风格，不直接 import CwStore，保持纯函数可测性）。 */
...
function renderDecisionsSection(unit: WorkUnitRecord): string[] {
  ...
  for (const c of clarifications) {
    if (c.resolution && c.resolution.trim() !== "") {
      lines.push(`- [design ${c.id}] ${c.question}`);
