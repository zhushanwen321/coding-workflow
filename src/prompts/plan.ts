...
plan gate 通过后状态从 created 流转到 planned。写完 plan.json 后调
cw(action=plan, topicId, planJson) 提交。
...
gate fail 时 CW 返回 mustFix（逐条 fail 原因），status 不变（仍 created），修 mustFix 后重调 cw(plan)。
