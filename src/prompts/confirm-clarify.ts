...
- [禁止] 不调 cw(gen-spec) 就直接 confirm（FR-8: confirm gate 校验 confirmSpec 存在，不调 gen-spec 会 gate fail）
- [禁止] gen-spec 后不等用户审查确认就 confirm（cw 已自动打开文档，但你要等用户说「确认/没问题」后才能 confirm_clarify）
- [禁止] 用户要修改但你不改就 confirm（回 clarify 追加/修改后再 gen-spec → 再确认）
