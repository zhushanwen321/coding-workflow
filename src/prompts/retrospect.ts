...
或 type=oneOff（一次性失误，仅记录）。至少 1 条 type=pattern。
...
- **knownRisks**：交付时已知但未完全解决的风险（设计级优先）
  ...
  - area：涉及的模块/功能区域
  - level：design（架构/接口/数据流层面的设计级风险）或 code（代码级细节风险）
  - description：具体风险描述，设计级需说明抽象层面的问题
  - unverified：是否为未经证实的假设（待 post-closeout 验证）
- **processIssues**：本次 topic 暴露的流程问题（供 quality-criteria 迭代参考）
  - 对象数组，每条带 type 分类
  - type=pattern：可泛化流程模式（跨 topic 可复现，有迭代价值）——至少 1 条
...
- [禁止] knownRisks 只列代码细节（设计级风险优先，标 level=design）
